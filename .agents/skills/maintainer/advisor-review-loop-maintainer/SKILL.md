...
description: "Use when Rudder development work needs first-principles advisor analysis plus independent reviewer rounds: proposals, UI/product decisions, architecture, release readiness, workflow changes, agent outcomes, explicit acceptance gates, adversarial or heuristic review, repeated review, or “没有通过 review 返工”."
---
...
Run a decision-grade advisor-to-reviewer loop for Rudder work that should not be
accepted after one author pass. This skill owns the advisor artifact and the
review loop; it does not replace implementation, black-box product verification,
or release execution skills.
...
- the user asks for adversarial, heuristic, product-systems, or multi-lens reviewer pressure
...
| High-stakes proposal, workflow, skill, architecture, release, or prior failed review | Use functional, adversarial, and heuristic/product-systems lenses |
| Narrow proposal or mechanical skill/doc change | Use the smallest two-lens set and record the omitted lens |
...
3. Spawn independent reviewers only after there is an artifact to judge. For consequential work, use three distinct lenses: functional trust, adversarial, and heuristic/product-systems.
...
| Asking multiple reviewers to run the same checklist | Give each reviewer a distinct functional, adversarial, or heuristic lens. |
