...
Capture a small non-secret ledger. Values should be statuses, timestamps, IDs,
URLs, process identities, or hashes, never tokens, cookies, email addresses, or
passwords.
...
Do not skip earlier stages because a later API happens to respond. When a
device authorization is pending or expired, report the exact missing approval
and stop. Do not retry by reading or exporting credentials.
...
by the target environment and require a fresh approval, exchange, claim, and
post-login window. A browser session at the local API does not automatically
authenticate the Electron shell.
...
Do not collapse a protected-request `401` into “runtime not ready”. Preserve
the distinction in the report even when the eventual fix is a single fetch
injection or readiness gate.
...
4. observe the native dialog surface with System Events/Computer Use or state
   that the required observation authority is unavailable;
