...
export function homePage(providers: { google: boolean; github: boolean }): string {
  const socialButtons = [
    providers.google ? `
         <button class="social-button" type="button" data-social="google">
    ...
    providers.github ? `
         <button class="social-button" type="button" data-social="github">
  ...
  const socialSection = socialButtons
    ? `<div class="social-stack" role="group" aria-label="Social sign in">${socialButtons}
       </div>
  ...
  return page(
    ...
    `<section class="auth-card" aria-labelledby="sign-in-heading">
       ...
       <p class="auth-intro">Sign in or create an account. Your Local Workspace stays on your machine.</p>
       ${socialSection}
       <form class="auth-form" id="otp-form">
         ...
         <button class="primary-button" type="submit">Continue with email</button>
       </form>
