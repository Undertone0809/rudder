...
function createTempConfig(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-doctor-"));
  const configPath = path.join(root, ".rudder", "config.json");
...
describe("doctor", () => {
  ...
  it("re-runs repairable checks so repaired failures do not remain blocking", async () => {
    ...
    expect(process.env.RUDDER_AGENT_JWT_SECRET).toBeTruthy();
  });
});
