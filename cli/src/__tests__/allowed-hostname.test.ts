...
function writeBaseConfig(configPath: string) {
  const base: RudderConfig = {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/rudder-db",
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/rudder-storage" },
      s3: {
        bucket: "rudder",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/rudder-secrets/master.key" },
    },
  };
...
describe("allowed-hostname command", () => {
  it("adds and normalizes hostnames", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf-8")) as RudderConfig;
