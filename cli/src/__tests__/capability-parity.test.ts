...
describe("CLI automation/chat/runs parity", () => {
  ...
  it("prints chat message run linkage and follow-up run commands in human output", async () => {
    ...
    expect(output.stdoutText()).toContain("runId=run_609695f1");
    expect(output.stdoutText()).not.toContain("runId=609695f1-f90a-4b17-be61-4f0c6fe37c42");
    expect(output.stdoutText()).toContain("runCommand=rudder runs get run_609695f1");
    expect(output.stdoutText()).toContain("transcriptCommand=rudder runs transcript run_609695f1");
