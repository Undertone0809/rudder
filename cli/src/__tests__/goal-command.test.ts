...
describe("Goal CLI commands", () => {
  beforeEach(() => {
    process.env = {
      ...
      RUDDER_ORG_ID: "33333333-3333-4333-8333-333333333333",
    };
    vi.spyOn(process.stdout, "write").mockImplementation(() => true);
    vi.spyOn(console, "error").mockImplementation(() => undefined);
  });

  it("discovers active Goals owned by the runtime Agent by default", async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({ goals: [], count: 0 }), { status: 200 }));
    ...
    expect(new URL(url).pathname).toBe("/api/orgs/33333333-3333-4333-8333-333333333333/goals/assigned");
    expect(Object.fromEntries(new URL(url).searchParams)).toEqual({ lifecycle: "active", limit: "20" });
    expect(init.headers).toMatchObject({ authorization: "Bearer runtime-key" });
  });

  it("reads the owned Goal operating context", async () => {
    ...
    const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit];
    expect(new URL(url).pathname).toBe("/api/goals/goal-1/agent-context");
    expect(init.headers).toMatchObject({ authorization: "Bearer runtime-key" });
  });
  ...
  it("submits a reviewable Goal contract change without applying it", async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({
      id: "change-1",
      status: "pending",
    }), { status: 201 }));
    ...
    await expect(runCli([
      ...
      "--rationale",
      "External approval moved the feasible delivery date.",
      "--evidence-refs",
      '["artifact://goal/schedule-evidence"]',
      "--idempotency-key",
      "goal-change-1",
      "--api-base",
      "http://localhost:3100",
    ...
    const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit];
    expect(new URL(url).pathname).toBe("/api/goals/goal-1/change-proposals");
    expect(init.method).toBe("POST");
    expect(init.headers).toMatchObject({
      "x-rudder-agent-id": "11111111-1111-4111-8111-111111111111",
      "x-rudder-run-id": "22222222-2222-4222-8222-222222222222",
