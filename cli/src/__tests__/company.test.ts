...
describe("resolveCompanyImportApiPath", () => {
  it("uses organization-scoped preview route for existing-organization dry runs", () => {
    expect(
      resolveCompanyImportApiPath({
        dryRun: true,
    ...
    ).toBe("/api/orgs/company-123/imports/preview");
  });

  it("uses organization-scoped apply route for existing-organization imports", () => {
    expect(
      resolveCompanyImportApiPath({
        dryRun: false,
        targetMode: "existing_organization",
  ...
  it("throws when an existing-organization import is missing an org id", () => {
    expect(() =>
      resolveCompanyImportApiPath({
        dryRun: true,
        targetMode: "existing_organization",
...
describe("buildCompanyDashboardUrl", () => {
  it("preserves the configured base path when building a dashboard URL", () => {
    expect(buildCompanyDashboardUrl("https://rudder.example/app/", "PAP")).toBe(
      "https://rudder.example/app/PAP/dashboard",
    );
  });
});
...
describe("renderCompanyImportPreview", () => {
  it("summarizes the preview with counts, selection info, and truncated examples", () => {
    const preview: OrganizationPortabilityPreviewResult = {
      ...
      targetOrganizationId: "company-123",
      targetOrganizationName: "Imported Co",
    ...
    const rendered = renderCompanyImportPreview(preview, {
      sourceLabel: "GitHub: https://github.com/rudder/organizations/demo",
      targetLabel: "Imported Co (company-123)",
      infoMessages: ["Using claude-local adapter"],
    });

    expect(rendered).toContain("Include");
    expect(rendered).toContain("organization, projects, tasks, agents, skills");
...
describe("renderCompanyImportResult", () => {
  it("summarizes import results with created, updated, and skipped counts", () => {
    const rendered = renderCompanyImportResult(
      ...
      {
        targetLabel: "Imported Co (company-123)",
        organizationUrl: "https://rudder.example/PAP/dashboard",
        infoMessages: ["Using claude-local adapter"],
      },
    );

    expect(rendered).toContain("Organization");
    expect(rendered).toContain("https://rudder.example/PAP/dashboard");
...
describe("import selection catalog", () => {
  it("defaults to everything and keeps project selection separate from task selection", () => {
    ...
    expect(state.organization).toBe(true);
    ...
    expect(selectedFiles).toContain(".rudder.yaml");
