...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".rudder");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: RudderConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "rudder",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    RUDDER_HOME: paths.homeDir,
    RUDDER_INSTANCE_ID: paths.instanceId,
    RUDDER_CONFIG: paths.configPath,
    RUDDER_CONTEXT: paths.contextPath,
    RUDDER_IN_WORKTREE: "true",
    ...(branding?.name ? { RUDDER_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { RUDDER_WORKTREE_COLOR: branding.color } : {}),
  };
}
