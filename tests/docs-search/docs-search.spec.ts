...
test("renders and filters the localized changelog timeline", async ({ page }) => {
  ...
  for (const item of locales) {
    ...
    await expect(page.locator("h2#v0-7-11")).toBeVisible();
    await expect(page.locator('h2[id^="v0-"]')).toHaveCount(42);
    const latestUpdate = page.locator("h2#v0-7-11").locator(
      "xpath=ancestor::div[contains(@class, 'update-container')]",
    );
    ...
    await expect(updateFor("v0-7-11")).toBeVisible();
    ...
    await expect(updateFor("v0-7-11")).toBeHidden();
    ...
    await page.goto(item.route);
    const mobileLatestUpdate = page.locator("h2#v0-7-11").locator(
      "xpath=ancestor::div[contains(@class, 'update-container')]",
    );
    ...
    expect(overflows, `${item.route} changelog at 390px`).toBe(false);
  }
});
