...
test("issue comments require confirmation unless they direct a real Agent mention", async ({ page }) => {
  ...
  for (const status of ["paused", "terminated", "pending_approval"] as const) {
    const blockedAgentRes = await page.request.post(`/api/orgs/${organization.id}/agents`, {
      data: {
        name: `${status} Reopen Agent`,
        role: "pm",
        agentRuntimeType: "codex_local",
        agentRuntimeConfig: {
          command: E2E_CODEX_STUB,
    ...
    expect(blockedAgentRes.ok()).toBe(true);
    const blockedAgent = await blockedAgentRes.json() as { id: string };

    const blockedIssueRes = await page.request.post(`/api/orgs/${organization.id}/issues`, {
      data: {
        title: `Reopen with a ${status} Agent`,
        status: "done",
        priority: "medium",
        assigneeAgentId: blockedAgent.id,
      },
    });
    expect(blockedIssueRes.ok()).toBe(true);
    const blockedIssue = await blockedIssueRes.json() as { id: string; identifier: string | null };
    const blockedIssueRef = blockedIssue.identifier ?? blockedIssue.id;
    const blockedAgentUpdateRes = await page.request.patch(`/api/agents/${blockedAgent.id}`, {
      data: { status },
    });
    expect(blockedAgentUpdateRes.ok()).toBe(true);

    await page.goto(`/${organization.issuePrefix}/issues/${blockedIssueRef}`);
    const blockedActivity = page.getByRole("region", { name: "Activity" });
    const blockedComposer = blockedActivity.locator(".rudder-milkdown-content [contenteditable='true']").last();
    const blockedCommentButton = blockedActivity.getByRole("button", { name: "Comment" }).last();
    await expect(blockedActivity.getByRole("checkbox", { name: "Re-open" })).toBeChecked();
    await blockedComposer.click();
    await page.keyboard.type(`Confirm the ${status} reopen first`);
