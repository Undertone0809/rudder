...
import { organizations, organizationMemberships, instanceUserRoles } from "@rudder/db";
...
export async function claimBoardOwnership(
  ...
  await db.transaction(async (tx) => {
    ...
    const allCompanies = await tx.select({ id: organizations.id }).from(organizations);
    for (const organization of allCompanies) {
      const existing = await tx
        ...
        .where(
          and(
            ...
            eq(organizationMemberships.principalId, opts.userId),
          ),
        )
        .then((rows) => rows[0] ?? null);
