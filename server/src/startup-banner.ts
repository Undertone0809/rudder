...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.RUDDER_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.RUDDER_AGENT_JWT_SECRET === "string" ? parsed.RUDDER_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm rudder onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const localEnvProfile = process.env.RUDDER_LOCAL_ENV?.trim() || null;
  const instanceId = resolveRudderInstanceId();
  const configPath = resolveRudderConfigPath();
  const envFilePath = resolveRudderEnvPath();
  ...
  const lines = [
    ...
    row("Local Env", localEnvProfile ? `${localEnvProfile} ${color(`(instance: ${instanceId})`, "dim")}` : instanceId),
