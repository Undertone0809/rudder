...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Rudder OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Rudder base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("rudderApiUrl");
    expect(text).toContain("agentRuntimeType \"openclaw_gateway\"");
    expect(text).toContain("headers.x-openclaw-token");
    expect(text).toContain("Do NOT use /v1/responses or /hooks/*");
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.rudderApiUrl");
    expect(text).toContain("~/.openclaw/workspace/rudder-claimed-api-key.json");
