...
describe("opencode execute", { timeout: 20_000 }, () => {
  ...
  it("injects organization-library runtime skills into the OpenCode prompt from the managed sidecar", async () => {
    ...
    try {
      ...
      expect(managedConfig.mcp?.["rudder-tools"]).toMatchObject({
        type: "local",
        enabled: true,
        environment: {
      ...
      expect(managedConfig.mcp?.["rudder-tools"]?.command).toEqual(
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        expect(meta.rudderMcp).toMatchObject({ available: true, serverName: "rudder-tools", toolCount: 72 });
