...
describe("cli auth routes", () => {
  ...
  it("approves a CLI auth challenge for a signed-in board user", async () => {
    mockBoardAuthService.approveCliAuthChallenge.mockResolvedValue({
      ...
      challenge: {
        ...
        requestedAccess: "board",
        requestedCompanyId: "organization-1",
        expiresAt: new Date("2026-03-23T13:00:00.000Z"),
      },
    });
  ...
  it("logs approve activity for instance admins without organization memberships", async () => {
    mockBoardAuthService.approveCliAuthChallenge.mockResolvedValue({
      status: "approved",
      challenge: {
        id: "challenge-2",
    ...
    mockBoardAuthService.resolveBoardActivityCompanyIds.mockResolvedValue(["organization-a", "organization-b"]);
  ...
  it("logs revoke activity with resolved audit organization ids", async () => {
    ...
    mockBoardAuthService.resolveBoardActivityCompanyIds.mockResolvedValue(["organization-z"]);
