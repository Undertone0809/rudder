...
import type { QuotaWindow } from "@rudder/agent-runtime-utils";
...
describe("readClaudeToken", () => {
  ...
  it("returns null for malformed JSON", async () => {
    const tmpDir = path.join(os.tmpdir(), `rudder-test-claude-${Date.now()}`);
...
describe("readCodexAuthInfo", () => {
  ...
  it("returns null for malformed JSON", async () => {
    const tmpDir = path.join(os.tmpdir(), `rudder-test-codex-${Date.now()}`);
    await import("node:fs/promises").then((fs) =>
      fs.mkdir(tmpDir, { recursive: true }).then(() =>
