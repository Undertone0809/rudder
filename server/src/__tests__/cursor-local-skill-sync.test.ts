...
describe("cursor local skill sync", () => {
  ...
  function managedCursorSkillsHome(home: string, orgId = "organization-1") {
    return path.join(home, ".rudder", "instances", "default", "organizations", orgId, "cursor-home", ".cursor", "skills");
  }
  ...
  it("recognizes organization-library runtime skills supplied outside the bundled Rudder directory", async () => {
    ...
    const installedEntry = after.entries.find((entry) => entry.key === "ascii-heart");
    expect(installedEntry?.state).toBe("installed");
    expect(installedEntry?.targetPath).toBe(path.join(managedCursorSkillsHome(home), "ascii-heart"));
    expect((await fs.lstat(installedEntry?.targetPath ?? "")).isSymbolicLink()).toBe(true);
  });
  ...
  it("removes Rudder-managed symlinks when the desired set is emptied", async () => {
    ...
    expect(targetPath).toContain(managedCursorSkillsHome(home));
