...
const mocks = vi.hoisted(() => ({
  ...
  startEntityCleanupWorker: vi.fn(),
  stopEntityCleanupWorker: vi.fn(),
  warn: vi.fn(),
}));
...
vi.mock("../services/entity-cleanup-jobs.js", () => ({
  startEntityCleanupWorker: mocks.startEntityCleanupWorker,
}));
...
describe("createRudderApp lifecycle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.startEntityCleanupWorker.mockReturnValue(mocks.stopEntityCleanupWorker);
  });
  ...
  it("rolls back the plugin runtime when HTTP app creation fails", async () => {
    ...
    expect(mocks.stopEntityCleanupWorker).toHaveBeenCalledTimes(1);
  });
