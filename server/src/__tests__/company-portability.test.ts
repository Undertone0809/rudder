...
vi.mock("../services/orgs.js", () => ({
  organizationService: () => companySvc,
}));
...
vi.mock("../services/automations.js", () => ({
  automationService: () => automationSvc,
}));

vi.mock("../services/organization-skills.js", () => ({
  organizationSkillService: () => organizationSkillSvc,
}));
...
describe("organization portability", () => {
  ...
  beforeEach(() => {
    ...
    automationSvc.list.mockResolvedValue([]);
    automationSvc.getDetail.mockImplementation(async (id: string) => {
      const rows = await automationSvc.list();
      return rows.find((row: { id: string }) => row.id === id) ?? null;
    });
    automationSvc.create.mockImplementation(async (_companyId: string, input: Record<string, unknown>) => ({
    ...
    automationSvc.createTrigger.mockImplementation(async (_automationId: string, input: Record<string, unknown>) => ({
      id: "trigger-created",
      orgId: "organization-1",
    ...
    const organizationSkills = [
      ...
      },
      {
        ...
        slug: "organization-playbook",
        name: "organization-playbook",
        description: "Internal organization skill",
        markdown: "---\nname: organization-playbook\ndescription: Internal organization skill\n---\n\n# Organization Playbook\n",
        sourceType: "local_path",
        sourceLocator: "/tmp/organization-playbook",
        sourceRef: null,
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [
    ...
    organizationSkillSvc.list.mockResolvedValue(organizationSkills);
    organizationSkillSvc.listFull.mockResolvedValue(organizationSkills);
    organizationSkillSvc.getEnabledSkillKeysForAgent.mockImplementation(
      async (_orgId: string, agent: { agentRuntimeConfig?: Record<string, unknown> | null }) => {
        const sync = agent?.agentRuntimeConfig?.rudderSkillSync;
        return Array.isArray(sync?.desiredSkills)
          ? sync.desiredSkills.filter((value): value is string => typeof value === "string")
          : [];
      },
    );
    organizationSkillSvc.replaceEnabledSkillKeysForAgent.mockResolvedValue([]);
    organizationSkillSvc.readFile.mockImplementation(async (_companyId: string, skillId: string, relativePath: string) => {
      if (skillId === "skill-2") {
        return {
          skillId,
          path: relativePath,
    ...
    organizationSkillSvc.importPackageFiles.mockResolvedValue([]);
    ...
    agentInstructionsSvc.materializeManagedBundle.mockImplementation(async (agent: { agentRuntimeConfig: Record<string, unknown> }) => ({
      bundle: null,
      agentRuntimeConfig: {
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/rudder/orgs?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "rudder",
      repo: "orgs",
      ref: "feature/demo",
      basePath: "gstack",
      companyPath: "gstack/ORGANIZATION.md",
    });
  });
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/rudder/orgs?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "rudder",
      repo: "orgs",
  ...
  it("exports referenced skills as stubs by default with sanitized Rudder extension data", async () => {
    ...
    expect(asTextFile(exported.files["ORGANIZATION.md"])).toContain('name: "Rudder"');
    expect(asTextFile(exported.files["ORGANIZATION.md"])).toContain('schema: "agentorganizations/v1"');
    ...
    expect(extension).toContain('schema: "rudder/v1"');
    ...
    expect(extension).not.toContain("rudderSkillSync");
  ...
  it("exports default sidebar order into the Rudder extension and manifest", async () => {
    ...
    expect(asTextFile(exported.files[".rudder.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("exports the organization logo into images/ and references it from .rudder.yaml", async () => {
    ...
    assetSvc.getById.mockResolvedValue({
      id: "logo-1",
      orgId: "organization-1",
      objectKey: "assets/orgs/logo-1",
      contentType: "image/png",
      originalFilename: "logo.png",
    });

    const portability = organizationPortabilityService({} as any, storage as any);
    ...
    expect(storage.getObject).toHaveBeenCalledWith("organization-1", "assets/orgs/logo-1");
    expect(exported.files["images/organization-logo.png"]).toEqual({
      encoding: "base64",
      data: Buffer.from("png-bytes").toString("base64"),
      contentType: "image/png",
    });
    expect(exported.files[".rudder.yaml"]).toContain('logoPath: "images/organization-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    organizationSkillSvc.listFull.mockResolvedValue([
      {
        id: "skill-local",
        orgId: "organization-1",
      ...
      },
      {
        id: "skill-rudder",
        orgId: "organization-1",
        key: "rudder/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/Undertone0809/rudder/tree/master/.agents/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          ...
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
          repoSkillDir: ".agents/skills/release-changelog",
        },
      },
    ]);
    ...
    expect(asTextFile(exported.files["skills/rudder/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/rudder/release-changelog/SKILL.md"])).toContain("rudder/release-changelog");
  });
  ...
  it("builds export previews without tasks by default", async () => {
    ...
    const preview = await portability.previewExport("organization-1", {
      include: {
        organization: true,
        agents: true,
        projects: true,
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/rudder",
            repoUrl: "https://github.com/Undertone0809/rudder.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .rudder-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/Undertone0809/rudder.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/rudder");
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = organizationPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "rudder-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/Undertone0809/rudder.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".rudder.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/Undertone0809/rudder.git"');
    expect(extension).toContain('projectWorkspaceKey: "rudder"');
  ...
  it("reads env inputs back from .rudder.yaml during preview import", async () => {
    const portability = organizationPortabilityService({} as any);
  ...
  it("exports automations as recurring task packages with Rudder automation extensions", async () => {
    const portability = organizationPortabilityService({} as any);
    ...
    const extension = asTextFile(exported.files[".rudder.yaml"]);
    expect(extension).toContain("automations:");
  ...
  it("imports recurring task packages as automations instead of one-time issues", async () => {
    const portability = organizationPortabilityService({} as any);
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "rudder-demo", files },
      include: { organization: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_organization", newOrganizationName: "Imported Rudder" },
      agents: "all",
      collisionStrategy: "rename",
    });
    ...
    expect(preview.plan.issuePlans).toEqual([
      expect.objectContaining({
        slug: "monday-review",
        reason: "Recurring task will be imported as an automation.",
      }),
    ]);
    ...
    expect(automationSvc.create).toHaveBeenCalledWith("organization-imported", expect.objectContaining({
    ...
    expect(automationSvc.createTrigger).toHaveBeenCalledTimes(2);
    expect(automationSvc.createTrigger).toHaveBeenCalledWith("automation-created", expect.objectContaining({
      kind: "schedule",
      cronExpression: "0 9 * * 1",
      timezone: "America/Chicago",
    }), expect.any(Object));
  ...
  it("migrates legacy schedule.recurrence imports into automation triggers", async () => {
    const portability = organizationPortabilityService({} as any);
    ...
    const files = {
      "ORGANIZATION.md": ['---', 'schema: "agentorganizations/v1"', 'name: "Imported Rudder"', "---", ""].join("\n"),
  ...
  it("flags recurring task imports that are missing automation-required fields", async () => {
    const portability = organizationPortabilityService({} as any);

    const preview = await portability.previewImport({
      source: {
        type: "inline",
        rootPath: "rudder-demo",
        files: {
          "ORGANIZATION.md": ['---', 'schema: "agentorganizations/v1"', 'name: "Imported Rudder"', "---", ""].join("\n"),
          "tasks/monday-review/TASK.md": [
            "---",
      ...
      include: { organization: true, agents: false, projects: false, issues: true, skills: false },
      target: { mode: "new_organization", newOrganizationName: "Imported Rudder" },
      collisionStrategy: "rename",
    });

    expect(preview.errors).toContain("Recurring task monday-review must declare a project to import as an automation.");
    expect(preview.errors).toContain("Recurring task monday-review must declare an assignee to import as an automation.");
  });

  it("imports a vendor-neutral package without .rudder.yaml", async () => {
    const portability = organizationPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.organization?.name).toBe("Imported Rudder");
  ...
  it("imports packaged skills and restores desired skill refs on agents", async () => {
    ...
    expect(agentSvc.create).toHaveBeenCalledWith("organization-imported", expect.objectContaining({
      agentRuntimeConfig: expect.not.objectContaining({
        rudderSkillSync: expect.anything(),
      }),
    }));
    expect(organizationSkillSvc.replaceEnabledSkillKeysForAgent).toHaveBeenCalledWith(
  ...
  it("imports a packaged organization logo and attaches it to the target organization", async () => {
    const storage = {
      putFile: vi.fn().mockResolvedValue({
        provider: "local_disk",
        objectKey: "assets/orgs/imported-logo",
        contentType: "image/png",
        byteSize: 9,
        sha256: "logo-sha",
        originalFilename: "organization-logo.png",
      }),
    };
    companySvc.create.mockResolvedValue({
      id: "organization-imported",
    ...
    exported.files["images/organization-logo.png"] = {
      encoding: "base64",
      data: Buffer.from("png-bytes").toString("base64"),
      contentType: "image/png",
    };
    exported.files[".rudder.yaml"] = `${exported.files[".rudder.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
      'brandColor: "#5c5fff"\n  logoPath: "images/organization-logo.png"\n',
    );
    ...
    expect(assetSvc.create).toHaveBeenCalledWith("organization-imported", expect.objectContaining({
      objectKey: "assets/orgs/imported-logo",
      contentType: "image/png",
      createdByUserId: "user-1",
    }));
    expect(companySvc.update).toHaveBeenCalledWith("organization-imported", {
      logoAssetId: "asset-created",
    });
  });
  ...
  it("imports only selected files and leaves unchecked organization metadata alone", async () => {
    ...
    expect(organizationSkillSvc.importPackageFiles).toHaveBeenCalledWith(
      "organization-1",
      expect.objectContaining({
        "ORGANIZATION.md": expect.any(String),
    ...
    expect(agentSvc.create).toHaveBeenCalledWith("organization-1", expect.objectContaining({
      name: "CMO",
      runtimeConfig: {
        heartbeat: {
          enabled: false,
    ...
    expect(result.organization.action).toBe("unchanged");
