...
describe("project workspace skill discovery", () => {
  ...
  it("finds bounded skill roots under supported workspace paths", async () => {
    const workspace = await makeTempDir("rudder-skill-workspace-");
    ...
    await writeSkillDir(path.join(workspace, "skills", ".system", "rudder"), "Rudder");
    ...
    expect(discovered).toEqual([
      ...
      { skillDir: path.resolve(workspace, "skills", ".system", "rudder"), inventoryMode: "full" },
  ...
  it("limits root SKILL.md imports to skill-related support folders", async () => {
    const workspace = await makeTempDir("rudder-root-skill-");
  ...
  it("parses inline object array items in skill frontmatter metadata", async () => {
    const workspace = await makeTempDir("rudder-inline-skill-yaml-");
  ...
  it("parses YAML block-scalar descriptions from SKILL frontmatter", async () => {
    const workspace = await makeTempDir("rudder-block-scalar-skill-");
    await fs.mkdir(workspace, { recursive: true });
    await fs.writeFile(
      path.join(workspace, "SKILL.md"),
      [
...
describe("organization skill local scans", () => {
  it("defaults to the organization skill workspace instead of the user home .agents directory", async () => {
    const previousHome = process.env.HOME;
    const previousRudderHome = process.env.RUDDER_HOME;
    const home = await makeTempDir("rudder-user-home-skills-");
    const rudderHome = await makeTempDir("rudder-org-scan-home-");
    ...
    try {
      ...
      await writeSkillDir(
        path.join(home, ".agents", "skills", "global-only"),
        "Global Only",
      );
      await writeSkillDir(
        path.join(
      ...
      const handlers = createOrganizationSkillScanHandlers({
        ...
        upsertImportedSkills: async (nextOrgId, imported) => imported.map((skill, index) => {
          persistedSlugs.push(skill.slug);
          return {
            ...skill,
            id: `skill-${index}`,
            orgId: nextOrgId,
      ...
      expect(result.imported.map((skill) => skill.slug)).toEqual(["org-only"]);
      expect(persistedSlugs).toEqual(["org-only"]);
...
describe("missing local skill reconciliation", () => {
  it("flags local-path skills whose directory was removed", async () => {
    const workspace = await makeTempDir("rudder-missing-skill-dir-");
  ...
  it("flags local-path skills whose SKILL.md file was removed", async () => {
    const workspace = await makeTempDir("rudder-missing-skill-file-");
    const skillDir = path.join(workspace, "skills", "ghost");
    await writeSkillDir(skillDir, "Ghost");
