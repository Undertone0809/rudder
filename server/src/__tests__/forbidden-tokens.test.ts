...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "rudder", LOGNAME: "rudder", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "rudder" }),
      },
    );

    expect(tokens).toEqual(["rudder", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "rudder" }, {
        userInfo: () => ({ username: "rudder" }),
      });

      expect(tokens).toEqual(["rudder", "custom-token"]);
    } finally {
