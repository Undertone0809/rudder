...
function ensureCompanyPrefix(orgId: string, objectKey: string): void {
...
function buildObjectKey(orgId: string, namespace: string, originalFilename: string | null): string {
...
export function createStorageService(provider: StorageProvider): StorageService {
  return {
    ...
    async putFile(input: PutFileInput): Promise<PutFileResult> {
      ...
      const objectKey = buildObjectKey(input.orgId, input.namespace, input.originalFilename);
    ...
    async getObject(orgId: string, objectKey: string) {
      ensureCompanyPrefix(orgId, objectKey);
      return provider.getObject({ objectKey });
    },

    async headObject(orgId: string, objectKey: string) {
      ensureCompanyPrefix(orgId, objectKey);
      return provider.headObject({ objectKey });
    },

    async deleteObject(orgId: string, objectKey: string) {
      ensureCompanyPrefix(orgId, objectKey);
