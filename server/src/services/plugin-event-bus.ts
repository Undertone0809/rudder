...
function passesFilter(event: PluginEvent, filter: EventFilter | null): boolean {
  ...
  if (filter.orgId !== undefined) {
    if (event.orgId !== filter.orgId) return false;
  }
...
export function createPluginEventBus(): PluginEventBus {
  ...
  function forPlugin(pluginId: string): ScopedPluginEventBus {
    return {
       ...
       */
      async emit(name: string, orgId: string, payload: unknown): Promise<PluginEventBusEmitResult> {
        if (!name || name.trim() === "") {
...
export interface ScopedPluginEventBus {
   ...
   * @param orgId  UUID of the organization this event belongs to.
   ...
   */
  emit(name: string, orgId: string, payload: unknown): Promise<PluginEventBusEmitResult>;
