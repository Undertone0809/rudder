 ...
 * enforcement service.
 *
 * @see doc/product/domains/governance-and-visibility/approvals-budgets-activity.md - budget visibility and hard-stop behavior
...
function normalizeScopeName(scopeType: BudgetScopeType, name: string) {
  if (scopeType === "organization") return name;
...
async function computeObservedAmount(
  db: Db,
  policy: Pick<PolicyRow, "orgId" | "scopeType" | "scopeId" | "windowKind" | "metric">,
) {
  if (policy.metric !== "billed_cents") return 0;

  const conditions = [eq(costEvents.orgId, policy.orgId)];
...
export function budgetService(db: Db, hooks: BudgetServiceHooks = {}) {
  ...
  async function listPolicyRows(orgId: string) {
    return db
      .select()
      .from(budgetPolicies)
  ...
  return {
    listPolicies: async (orgId: string): Promise<BudgetPolicy[]> => {
      const rows = await listPolicyRows(orgId);
      return rows
        .filter((row) => row.scopeType === "organization" || row.scopeType === "agent")
        .map((row) => ({
          ...row,
          scopeType: row.scopeType as BudgetScopeType,
          metric: row.metric as BudgetMetric,
    ...
    upsertPolicy: async (
      ...
      if (scope.orgId !== orgId) {
        throw unprocessable("Budget scope does not belong to organization");
      }
    ...
    deletePolicy: async (
      ...
      return db.transaction(async (tx) => {
        ...
        if (
          policy.orgId !== orgId
          || !policy.isActive
          || (policy.scopeType !== "organization" && policy.scopeType !== "agent")
        ) {
    ...
    overview: async (orgId: string): Promise<BudgetOverview> => {
      ...
      const activeRows = rows.filter(
        (row) => row.isActive && (row.scopeType === "organization" || row.scopeType === "agent"),
      );
      ...
      const activeIncidentRows = await db
        ...
        .from(budgetIncidents)
        .where(and(
          eq(budgetIncidents.orgId, orgId),
          eq(budgetIncidents.status, "open"),
          inArray(budgetIncidents.scopeType, ["organization", "agent"]),
        ))
    ...
    evaluateCostEvent: async (event: typeof costEvents.$inferSelect) => {
      const candidatePolicies = await db
        ...
        .where(
          and(
            eq(budgetPolicies.orgId, event.orgId),
            eq(budgetPolicies.isActive, true),
            inArray(budgetPolicies.scopeType, ["organization", "agent"]),
          ),
        );

      const relevantPolicies = candidatePolicies.filter((policy) => {
        if (policy.scopeType === "organization") return policy.scopeId === event.orgId;
    ...
    getInvocationBlock: async (
      ...
      if (organization.status === "paused") {
        return {
          scopeType: "organization" as const,
          scopeId: orgId,
          scopeName: organization.name,
          reason:
            organization.pauseReason === "budget"
              ? "Organization is paused because its budget hard-stop was reached."
              : "Organization is paused and cannot start new work.",
        };
      }
      ...
      const companyPolicy = await db
        ...
        .where(
          and(
            eq(budgetPolicies.orgId, orgId),
            eq(budgetPolicies.scopeType, "organization"),
            eq(budgetPolicies.scopeId, orgId),
            eq(budgetPolicies.isActive, true),
      ...
      if (companyPolicy && companyPolicy.hardStopEnabled && companyPolicy.amount > 0) {
        ...
        if (observed >= companyPolicy.amount) {
          return {
            ...
            scopeId: orgId,
            scopeName: organization.name,
            reason: "Organization cannot start new work because its budget hard-stop is exceeded.",
          };
        }
      }
    ...
    resolveIncident: async (
      ...
      if (incident.orgId !== orgId) throw notFound("Budget incident not found");
      if (incident.scopeType !== "organization" && incident.scopeType !== "agent") {
      ...
      if (
        policy.orgId !== orgId
        || policy.scopeType !== incident.scopeType
        || policy.scopeId !== incident.scopeId
        || (policy.scopeType !== "organization" && policy.scopeType !== "agent")
      ) {
        throw notFound("Budget incident not found");
      }
