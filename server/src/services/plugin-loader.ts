...
export function isPluginPackageName(name: string): boolean {
  ...
  // Also accept scoped packages like @acme/plugin-linear or @rudder/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "rudderPlugin" key in package.json with a "manifest"
...
export function pluginLoader(
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @rudder/shared exports). Run those workers through
