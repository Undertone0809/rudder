...
async function stageIssueDescriptionAssets(input: {
  ...
  for (const sourceAssetId of sourceAssetIds) {
    ...
    if (!sourceAsset) {
      throw unprocessable("Issue description image asset is missing or outside the organization");
    }
    ...
    try {
      ...
      if (body.length === 0) throw new Error("Issue description image asset is empty");
      ...
      throw unprocessable("Issue description image could not be saved; please retry the Issue creation");
    }
...
export function issueService(db: Db, storage?: StorageService) {
  ...
  async function assertValidProjectWorkspace(orgId: string, projectId: string | null | undefined, projectWorkspaceId: string) {
    ...
    const workspace = await db
      ...
      .then((rows) => rows[0] ?? null);
    if (!workspace) throw notFound("Project run directory not found");
    if (workspace.orgId !== orgId) throw unprocessable("Project run directory must belong to same organization");
    if (projectId && workspace.projectId !== projectId) {
      throw unprocessable("Project run directory must belong to the selected project");
    }
  }
  ...
  const service = {
    ...
    create: async (
      ...
      try {
        ...
        const sourceAssetIds = extractIssueDescriptionAssetIds(issueData.description);
        if (sourceAssetIds.length > 0 && !storage) {
          throw unprocessable("Issue description image storage is unavailable; please retry the Issue creation");
        }
