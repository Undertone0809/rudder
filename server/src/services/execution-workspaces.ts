...
function toRunWorkspace(row: ExecutionWorkspaceRow): RunWorkspace {
  return {
    id: row.id,
    orgId: row.orgId,
    ...
    mode: row.mode as RunWorkspace["mode"],
    strategyType: row.strategyType as RunWorkspace["strategyType"],
    name: row.name,
    status: row.status as RunWorkspace["status"],
    cwd: row.cwd ?? null,
    repoUrl: row.repoUrl ?? null,
    baseRef: row.baseRef ?? null,
    branchName: row.branchName ?? null,
    providerType: row.providerType as RunWorkspace["providerType"],
    providerRef: row.providerRef ?? null,
    derivedFromRunWorkspaceId: row.derivedFromExecutionWorkspaceId ?? null,
...
export function runWorkspaceService(db: Db) {
  return {
    list: async (orgId: string, filters?: {
      ...
      const conditions = [eq(executionWorkspaces.orgId, orgId)];
      ...
      return rows.map(toRunWorkspace);
    },
...
/** @deprecated Use runWorkspaceService. */
export const executionWorkspaceService = runWorkspaceService;
/** @deprecated Use toRunWorkspace. */
export const toExecutionWorkspace = toRunWorkspace;
export { toRunWorkspace };
