...
function jwtConfig() {
  const secret =
    process.env.RUDDER_AGENT_JWT_SECRET ??
  ...
  return {
    secret,
    ttlSeconds: parseNumber(process.env.RUDDER_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.RUDDER_AGENT_JWT_ISSUER ?? "rudder",
    audience: process.env.RUDDER_AGENT_JWT_AUDIENCE ?? "rudder-api",
  };
}
...
export function createLocalAgentJwt(agentId: string, orgId: string, agentRuntimeType: string, runId: string) {
...
export function verifyLocalAgentJwt(token: string): LocalAgentJwtClaims | null {
  ...
  const agentRuntimeType = typeof claims.adapter_type === "string" ? claims.adapter_type : null;
  const runId = typeof claims.run_id === "string" ? claims.run_id : null;
  const iat = typeof claims.iat === "number" ? claims.iat : null;
  const exp = typeof claims.exp === "number" ? claims.exp : null;
  if (!sub || !orgId || !agentRuntimeType || !runId || !iat || !exp) return null;
