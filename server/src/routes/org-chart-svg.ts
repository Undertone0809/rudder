/**
 * Server-side SVG renderer for Rudder org charts.
...
import { ORG_CHART_LOGO_DATA_URI } from "./org-chart-logo.js";
...
// Rudder watermark mark rendered as an embedded PNG so exported charts stay self-contained.
const RUDDER_LOGO_SVG = `<image href="${ORG_CHART_LOGO_DATA_URI}" width="28" height="28" preserveAspectRatio="xMidYMid meet"/>`;
...
export interface OrgChartOverlay {
  /** Organization name displayed top-left */
...
export function renderOrgChartSvg(orgTree: OrgNode[], style: OrgChartStyle = "warmth", overlay?: OrgChartOverlay): string {
  ...
  const logoX = TARGET_W - 28 - LOGO_PADDING;
  const logoY = LOGO_PADDING;

  // Optional overlay elements
  const overlayNameSvg = overlay?.organizationName
    ? `<text x="${LOGO_PADDING}" y="${LOGO_PADDING + 16}" font-family="'Inter', -apple-system, BlinkMacSystemFont, sans-serif" font-size="22" font-weight="700" fill="${theme.nameColor}">${svgEscape(overlay.organizationName)}</text>`
    : "";
  ...
  <g transform="translate(${logoX}, ${logoY})" opacity="0.32">
    ${RUDDER_LOGO_SVG}
  </g>
  ${overlayNameSvg}
  ${overlayStatsSvg}
