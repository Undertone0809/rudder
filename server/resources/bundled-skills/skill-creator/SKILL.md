...
### Choose the skill ownership and location first

Before you create files, decide whether this skill is global or project-local.

Inside Rudder, use the ownership model in `references/rudder.md` instead:

- A skill used only by the current agent lives in `$AGENT_HOME/skills/<skill-name>`.
- A skill shared by the team lives in `$RUDDER_ORG_SKILLS_DIR/<skill-name>` and
  must be imported into the organization Skill Library.
- Installation and runtime enablement are separate. Validate the package before
  enabling it, and do not treat provider-native or global discovery as proof
  that Rudder will load the skill.

Outside Rudder, use the generic host locations:
...
Outside Rudder, put results in `<skill-name>-workspace/` as a sibling to the
skill directory. Inside Rudder, use the durable eval workspace described in
`references/rudder.md`; never write eval results into the read-only bundled
package or a provider-managed runtime mount. Within the workspace, organize
results by iteration (`iteration-1/`, `iteration-2/`, etc.) and within that,
each test case gets a directory (`eval-0/`, `eval-1/`, etc.). Don't create all
of this upfront — just create directories as you go.
...
Inside Rudder, packaging does not install or enable the skill. Follow
`references/rudder.md` to register the full package and, when authorized,
enable its returned selection reference.
...
- `references/rudder.md` — Rudder ownership, installation, enablement, permissions, and eval workspace rules
