...
table from top to bottom and use the first matching row. The class controls the
default repository build intensity; required domain-specific checks still
apply.

| Class | Use when | Required local evidence |
| --- | --- | --- |
| `FULL_GATE` | The change crosses package or shared-contract boundaries; changes dependencies, production build configuration, generated code, schema/build/release artifacts, migrations, permissions, organization boundaries, runtime adapters, or release candidates; is a broad refactor; or has a blast radius that cannot be bounded reliably | Full lint, recursive typecheck, full tests, and repository build, plus every applicable specialized gate |
| `SPECIALIZED` | The scope is bounded, but the claim depends on a real UI, rendered public documentation, runtime, packaged Desktop app, database lifecycle, external process, or another domain surface that a generic build cannot prove | The owning domain's real validation plus the minimum relevant static and focused checks; no full repository build by default |
| `SCOPED` | Executable behavior changes inside one package or one bounded workflow without crossing a shared contract, build, packaging, migration, permission, or runtime boundary | Changed-file lint, owning-package typecheck, focused tests, and relevant E2E when a user-visible workflow changes |
| `NO_BUILD` | Contributor documentation, comments, prose, a refresh of non-public generated documentation from unchanged generation logic, or mechanical changes cannot affect executable behavior, production build inputs, packaging, or release artifacts | Scoped diff check and any applicable documentation or structure validator; no typecheck, test suite, or build by default |
...
For `FULL_GATE`, freeze the candidate and run the broad command set once:
...
omissions in the hand-off. See `doc/engineering/DEVELOPING.md` for the decision
procedure, focused command recipes, and examples.
...
the final diff, and obtain a reviewer final handoff verdict of `accept` for that
