#!/usr/bin/env bash
#
# Kill all local Rudder dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all rudder dev processes
...
# Collect PIDs of node processes running from any rudder directory.
# Matches paths like /Users/*/rudder/... or /Users/*/rudder-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/rudder(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No Rudder dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} Rudder dev process(es):"
echo ""
