...
Examples:
  ...
  ./scripts/clean-onboard-ref.sh v0.1.0
...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Rudder data dir to use
  RUDDER_HOST=127.0.0.1    Host passed to the onboarded server
  RUDDER_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/rudder-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/rudder-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export RUDDER_HOST="${RUDDER_HOST:-127.0.0.1}"
export RUDDER_PORT="${RUDDER_PORT:-3100}"
export RUDDER_OPEN_ON_LISTEN="${RUDDER_OPEN_ON_LISTEN:-false}"
...
echo "RUDDER_HOST: $RUDDER_HOST"
echo "RUDDER_PORT: $RUDDER_PORT"
...
env \
  ...
  HOST="$RUDDER_HOST" \
  PORT="$RUDDER_PORT" \
  RUDDER_OPEN_ON_LISTEN="$RUDDER_OPEN_ON_LISTEN" \
  pnpm rudder onboard --yes --data-dir "$PC_DATA"
