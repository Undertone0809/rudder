...
IMAGE_NAME="${IMAGE_NAME:-rudder-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
RUDDER_VERSION="${RUDDER_VERSION:-latest}"
...
RUDDER_DEPLOYMENT_MODE="${RUDDER_DEPLOYMENT_MODE:-authenticated}"
RUDDER_DEPLOYMENT_EXPOSURE="${RUDDER_DEPLOYMENT_EXPOSURE:-private}"
RUDDER_PUBLIC_URL="${RUDDER_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@rudder.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-rudder-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e RUDDER_DEPLOYMENT_MODE="$RUDDER_DEPLOYMENT_MODE" \
      -e RUDDER_DEPLOYMENT_EXPOSURE="$RUDDER_DEPLOYMENT_EXPOSURE" \
      -e RUDDER_PUBLIC_URL="$RUDDER_PUBLIC_URL" \
      -e RUDDER_HOME="/rudder" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "rudder@${RUDDER_VERSION}" auth bootstrap-ceo --data-dir "$RUDDER_HOME" --base-url "$RUDDER_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$RUDDER_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$RUDDER_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$RUDDER_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$RUDDER_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$RUDDER_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$RUDDER_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg RUDDER_VERSION="$RUDDER_VERSION" \
...
echo "    Public URL: $RUDDER_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $RUDDER_DEPLOYMENT_MODE/$RUDDER_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/rudder-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$RUDDER_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $RUDDER_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$RUDDER_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $RUDDER_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
