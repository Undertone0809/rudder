...
export const RELEASE_SET_COMPONENTS = Object.freeze([
  "electron",
  "rust-server",
  "cli",
  "mcp",
...
const COMPONENT_ARTIFACT_KINDS = new Set(RELEASE_SET_COMPONENTS);
...
function validateArtifact(artifact, manifestVersion) {
  const artifactKeys = ["id", "kind", "version", "sha256", "bytes", "platform", "arch"];
  assertExactKeys(
    artifact,
    Object.hasOwn(artifact, "component") ? [...artifactKeys, "component"] : artifactKeys,
    "artifact",
  );
  if (
  ...
  if (Object.hasOwn(artifact, "component")) {
    if (!RELEASE_SET_COMPONENTS.includes(artifact.component)) {
      fail("unknown_release_set_artifact", `unknown artifact component ${artifact.component}`);
    }
    if (COMPONENT_ARTIFACT_KINDS.has(artifact.kind) && artifact.kind !== artifact.component) {
      fail(
        "mixed_release_set_identities",
        `artifact ${artifact.id} kind ${artifact.kind} does not match component ${artifact.component}`,
      );
    }
  }
...
export function validateReleaseSetManifest(manifest, { expectedArtifactIds, expectedComponents } = {}) {
  ...
  if (expectedComponents !== undefined) {
    if (
      ...
      || expectedComponents.length < 1
      || new Set(expectedComponents).size !== expectedComponents.length
      || expectedComponents.some((component) => !RELEASE_SET_COMPONENTS.includes(component))
    ) {
      fail("release_set_component_mismatch", "expected components must be unique known release-set components");
    }
    const actualComponents = manifest.artifacts.map((artifact) => artifact.component);
    if (
      actualComponents.some((component) => component === undefined)
      || new Set(actualComponents).size !== actualComponents.length
      || actualComponents.length !== expectedComponents.length
      || [...actualComponents].sort(compareCanonicalText).some(
        (component, index) => component !== [...expectedComponents].sort(compareCanonicalText)[index],
      )
    ) {
      fail("release_set_component_mismatch", "artifact component identities differ from the expected release set");
    }
  }
...
export function verifySignedReleaseSet(envelope, trust) {
  ...
  const manifest = validateReleaseSetManifest(envelope.payload, {
    expectedArtifactIds: trust.expectedArtifactIds,
    expectedComponents: trust.expectedComponents,
  });
