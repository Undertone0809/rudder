...
RUDDER_API_URL="${RUDDER_API_URL:-http://127.0.0.1:3100}"
API_BASE="${RUDDER_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-rudder-smoke}"
...
RUDDER_API_URL_FOR_OPENCLAW="${RUDDER_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${RUDDER_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${RUDDER_AUTH_HEADER}" )
fi
if [[ -n "${RUDDER_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${RUDDER_COOKIE}" )
  RUDDER_BROWSER_ORIGIN="${RUDDER_BROWSER_ORIGIN:-${RUDDER_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${RUDDER_BROWSER_ORIGIN}" -H "Referer: ${RUDDER_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set RUDDER_COOKIE or RUDDER_AUTH_HEADER."
  fi
...
cleanup_openclaw_agents() {
  ...
  local ids
  ids="$(jq -r '.[] | select((.agentRuntimeType == "openclaw" or .agentRuntimeType == "openclaw_gateway")) | .id' <<<"$RESPONSE_BODY")"
  if [[ -z "$ids" ]]; then
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg rudderApiUrl "$RUDDER_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/rudder-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/rudder-claimed-api-key.json" >/dev/null 2>&1 || true
  fi
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set RUDDER_API_KEY=${AGENT_API_KEY} in your run context before running Rudder heartbeat steps."
  ...
  patch_payload="$(jq -c --arg message "$base_message" '
    {agentRuntimeConfig: ((.agentRuntimeConfig // {}) + {
      payloadTemplate: (((.agentRuntimeConfig // {}).payloadTemplate // {}) + {
        message: $message
      })
    })}
  ' <<<"$RESPONSE_BODY")"
...
validate_joined_gateway_agent() {
  ...
  adapter_type="$(jq -r '.agentRuntimeType // empty' <<<"$RESPONSE_BODY")"
  gateway_url="$(jq -r '.agentRuntimeConfig.url // empty' <<<"$RESPONSE_BODY")"
  configured_token="$(jq -r '.agentRuntimeConfig.headers["x-openclaw-token"] // .agentRuntimeConfig.headers["x-openclaw-auth"] // empty' <<<"$RESPONSE_BODY")"
  disable_device_auth="$(jq -r 'if .agentRuntimeConfig.disableDeviceAuth == true then "true" else "false" end' <<<"$RESPONSE_BODY")"
  device_key_len="$(jq -r '(.agentRuntimeConfig.devicePrivateKeyPem // "" | length)' <<<"$RESPONSE_BODY")"

  [[ "$adapter_type" == "openclaw_gateway" ]] || fail "joined agent agentRuntimeType is '${adapter_type}', expected 'openclaw_gateway'"
  [[ "$gateway_url" =~ ^wss?:// ]] || fail "joined agent gateway url is invalid: '${gateway_url}'"
  [[ -n "$configured_token" ]] || fail "joined agent missing agentRuntimeConfig.headers.x-openclaw-token"
...
patch_agent_session_strategy_run() {
  ...
  local patch_payload
  patch_payload="$(jq -c '{agentRuntimeConfig: ((.agentRuntimeConfig // {}) + {sessionKeyStrategy:"run"})}' <<<"$RESPONSE_BODY")"
  api_request "PATCH" "/agents/${AGENT_ID}" "$patch_payload"
  assert_status "200"
}
...
main() {
  ...
  wait_http_ready "${RUDDER_API_URL%/}/api/health" 15 || fail "Rudder API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "rudder health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
