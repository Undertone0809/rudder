...
SMOKE_IMAGE="${SMOKE_IMAGE:-rudder-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-rudder-openclaw-smoke}"
...
Provide one of:
  RUDDER_AUTH_HEADER=\"Bearer <board-token>\"
  RUDDER_COOKIE=\"<board-session-cookie>\"
...
if [[ -z "$ORG_ID" ]]; then
  ...
  assert_status "200"
  ORG_ID="$(jq -r '.[0].id // empty' <<<"$RESPONSE_BODY")"
  if [[ -z "$ORG_ID" ]]; then
    fail "no organizations found; create one before running smoke test"
  fi
fi

log "creating agent-only invite for organization ${ORG_ID}"
INVITE_PAYLOAD="$(jq -nc '{allowedJoinTypes:"agent"}')"
api_request "POST" "/orgs/${ORG_ID}/invites" "$INVITE_PAYLOAD"
...
assert_status "200"
if ! grep -q "Rudder OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
api_request "POST" "/orgs/${ORG_ID}/join-requests/${JOIN_REQUEST_ID}/approve" "{}"
if [[ "$RESPONSE_CODE" == "401" || "$RESPONSE_CODE" == "403" ]]; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.rudder.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
