...
RUDDER_HOST_PORT="${RUDDER_HOST_PORT:-3100}"
RUDDER_HOST_FROM_CONTAINER="${RUDDER_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.rudder-openclaw.override.yml"
...
detect_rudder_base_url() {
  ...
  for candidate in "$RUDDER_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${RUDDER_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${RUDDER_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
rudder_base_url="$(detect_rudder_base_url || true)"
...
Rudder URL for OpenClaw container:
EOF
  if [[ -n "$rudder_base_url" ]]; then
    cat <<EOF
  $rudder_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${RUDDER_HOST_PORT}
  (Do not use http://127.0.0.1:${RUDDER_HOST_PORT} inside the container.)
  If Rudder rejects the host, run on host machine:
    pnpm rudder allowed-hostname host.docker.internal
  Then restart Rudder and re-run this script.
EOF
  fi
  cat <<EOF
