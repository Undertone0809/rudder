...
describe("signed release-set protocol", () => {
  ...
  it("binds known Electron, Rust server, CLI, MCP, and helper component identities", () => {
    const payload = manifest();
    payload.artifacts = [
      { ...payload.artifacts[0], component: "cli" },
      { ...payload.artifacts[1], component: "electron" },
      {
        id: "mcp/rudder-mcp",
        kind: "mcp",
        component: "mcp",
      ...
      {
        id: "native/rudder-update-helper",
        kind: "helper",
        component: "helper",
        version: payload.version,
        sha256: "5".repeat(64),
    ...
    expect(validateReleaseSetManifest(payload, {
      expectedComponents: ["electron", "rust-server", "cli", "mcp", "helper"],
    })).toBe(payload);

    const unknown = structuredClone(payload);
    unknown.artifacts[0].component = "unknown-runtime";
    expectCode(() => validateReleaseSetManifest(unknown), "unknown_release_set_artifact");
  });
