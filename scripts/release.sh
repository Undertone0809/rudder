...
Notes:
  ...
  - Stable release notes must already exist at releases/v<version>.md and the
    matching Chinese Desktop notes must exist at releases/zh/v<version>.md.
    Both localized public changelog entries must also exist in docs/releases.mdx
    and docs/zh/releases.mdx.
  - --preflight checks the committed version, release notes, Desktop notes,
    public changelog, git tags, and npm versions without installing
    dependencies, building, or publishing.
...
if [ "$channel" = "stable" ]; then
  ...
  node "$REPO_ROOT/scripts/verify-desktop-release-notes.mjs" --version "$TARGET_STABLE_VERSION"
fi
