...
1. Does the change cross package or shared-contract boundaries, alter
   dependencies/production build configuration/generated code or
   schema/build/release artifacts, add a migration, affect permissions or
   organization isolation, change a runtime adapter or release candidate, form
   a broad refactor, or have an unbounded blast radius? Use `FULL_GATE`.
2. Does the bounded claim depend on a real rendered UI, rendered public
   documentation, running service, packaged Desktop app, database lifecycle,
   external process, or another domain surface that compilation and unit tests
   cannot establish? Use `SPECIALIZED`.
3. Does it change executable behavior inside one package or one bounded
   workflow? Use `SCOPED`.
4. Otherwise, when it cannot affect executable behavior, production build
   inputs, packaging, or release artifacts, use `NO_BUILD`. This includes a
   refresh of non-public generated documentation when its generator and
...
| Refresh of non-public generated documentation with unchanged generation logic | `NO_BUILD` | Scoped diff check and the generator's determinism or structure validator |
| Public docs, CSS, or documentation image change | `SPECIALIZED` | Docs validators plus rendered route, viewport, theme, and asset inspection |
...
- do not inject the full organization resource catalog by default; project-attached
  ...
  through Rudder when an agent needs broader background

## Data Volume and Render Performance

Treat production-shaped data volume as a development constraint from the start,
not as a late polish pass. Rudder is an operational Rudder; pages must
...
`scope: declared-only`. Declared Collaboration, Execution, and Rudder
...
When a task requires browser-based verification, prefer `$ego-browser` as the
