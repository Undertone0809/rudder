...
This document summarizes the memory systems referenced in task `PAP-530` and extracts the design patterns that matter for Rudder.

## What Rudder Needs From This Survey

Rudder is not trying to become a single opinionated memory engine. The more useful target is a Rudder memory surface that:
...
- records memory-related cost and latency the same way the rest of Rudder records work
...
These emphasize local persistence, inspectability, and low operational overhead. They are useful because Rudder is local-first today and needs at least one zero-config path.

## Per-Project Notes

| Project | Shape | Notable API / model | Strong fit for Rudder | Main mismatch |
|---|---|---|---|---|
...
| [EverMemOS](https://github.com/EverMind-AI/EverMemOS) | conversational long-term memory system | MemCell extraction, structured narratives, user profiles, hybrid retrieval / reranking | useful model for provenance-rich structured memories and evolving profiles | focused on conversational memory rather than generalized Rudder events |
...
If Rudder does not expose these, it will not adapt well to the systems above.

## Where The Systems Differ

These differences are exactly why Rudder needs a layered contract instead of a single hard-coded engine.
...
Rudder should support both:

- provider-managed extraction
- Rudder-managed extraction with provider-managed storage / retrieval
...
Rudder should not require a single storage shape. It should require normalized references back to Rudder entities.
...
Rudder should make plain search the minimum contract and richer outputs optional capabilities.
...
Rudder needs both direct request/response operations and background maintenance hooks.

## Rudder-Specific Takeaways

### Rudder should own these concerns

- binding a provider to a company and optionally overriding it per agent
- mapping Rudder entities into provider scopes
- provenance back to issue comments, documents, runs, and activity
- cost / token / latency reporting for memory work
- browse and inspect surfaces in the Rudder UI
...
### The Rudder contract should stay small
...
1. `Memory binding + product integration`
...
2. `Provider adapter layer`
   A built-in or plugin-supplied adapter turns Rudder memory requests into provider-specific calls.
...
without forcing Rudder itself to become a monolithic memory engine.
