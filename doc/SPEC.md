...
A Organization is a first-order object. One Rudder instance runs multiple Organizations. A Organization does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Every Organization has a **Board** that governs high-impact decisions. The Board is the human oversight layer.
...
- **Set and modify Organization budgets** — the Board sets top-level token/LLM cost budgets
...
- Organization lifecycle (pause, archive, delete)?
...
#### Rudder Protocol (What Rudder Knows)

At the protocol level, Rudder tracks:
...
A key goal: **the entire org's agent configurations are exportable.** You can export a organization's complete agent setup — every agent, their adapter configs, org structure — as a portable artifact. This enables:

- Sharing organization templates ("here's a pre-built marketing agency org")
- Version controlling your organization configuration
- Duplicating/forking organizations

#### Context Delivery

Configurable per agent. Two ends of the spectrum:

- **Fat payload** — Rudder bundles relevant context (current tasks, messages, organization state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to Rudder.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls Rudder's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a Rudder agent: **be callable.** That's it. Rudder can invoke you via command or webhook. No requirement to report back — Rudder infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, Rudder provides progressively richer integration:
...
1. **Template export** (default) — structure only: agent definitions, org chart, adapter configs, role descriptions. Optionally includes a few seed tasks to help get started. This is the blueprint for spinning up a new organization.
2. **Snapshot export** — full state: structure + current tasks, progress, agent status. A complete picture you could restore or fork.

The usual workflow: export a template, create a new organization from it, add a couple initial tasks, go.
...
The heartbeat is a protocol, not a runtime. Rudder defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how Rudder invokes the agent. Initial adapters:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets Rudder check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the Rudder REST API.

### What Rudder Controls
...
- Health monitoring — how does Rudder distinguish "stuck" from "working on a long task"?
...
Full hierarchy: **Initiative** (organization goal) → Projects → Milestones → Issues → Sub-issues. Everything traces back to an initiative, and the "organization goal" is just the first/primary initiative.

---

## 6. Cost Tracking [DRAFT]

Token/LLM cost budgeting is a core part of Rudder. External revenue and expense tracking is a future plugin.
...
1. **Visibility** — dashboards showing spend at every level (Agent, task, project, Organization)
...
How a Organization goes from "created" to "running":

1. Human creates a Organization and its initial Initiatives
...
Rudder ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **Rudder Skill** (SKILL.md) so it can interact with the task system, read Organization context, report status.
...
A skill definition that teaches agents how to interact with Rudder. Provides:

- Task CRUD (create, read, update, complete tasks)
- Status reporting (check in, report progress)
- Organization context (read goal, org chart, current state)
...
**Single-tenant, self-hostable.** Not a SaaS. One instance = one operator's organizations.
...
3. **Open organization** — Optionally make parts public (e.g. a job board visible to the public for open organizations).

The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my Rudder instance."

#### Agent Authentication

When a user creates an Agent, Rudder generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), Rudder sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the Rudder SKILL, but sometimes it's unavoidable.

### API Design

**Single unified REST API.** The same API serves both the frontend UI and agents. Authentication determines permissions — board auth has full access, agent API keys have scoped access (their own tasks, cost reporting, organization context).

No separate "agent API" vs. "board API." Same endpoints, different authorization levels.

### Work Artifacts

Rudder does **not** manage work artifacts (code repos, file systems, deployments, documents). That's entirely the agent's domain. Rudder tracks tasks and costs. Where and how work gets done is outside scope.

### Open Questions

- Real-time updates to the UI — WebSocket? SSE? Polling?
- Agent API key scoping — what exactly can an Agent access? Only their own tasks? Their team's? The whole Organization?

### Crash Recovery: Manual, Not Automatic

When an agent crashes or disappears mid-task, Rudder does **not** auto-reassign or auto-release the task. Instead:

- Rudder surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- Rudder does not fail silently — the auditing and visibility tools make problems obvious
...
- Rudder may create a visible same-agent recovery retry for a known failed run, but that retry must remain explicit, traceable, and non-silent

**Principle: Rudder reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core Rudder system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
The plugin framework has shipped. Plugins can register new adapter types, hook into lifecycle events, and contribute UI components (e.g. global toolbar buttons). A plugin SDK and CLI commands (`rudder plugin`) are available for authoring and installing plugins.
...
1. **Organization Structure** — the org tree with live status indicators (running/idle/paused/error) per agent. Real-time activity feed of what agents are doing.
...
6. **Cost Dashboard** — spend visualization at every level (agent, task, project, organization).
...
**Full loop with one adapter.** V1 must demonstrate the complete Rudder cycle end-to-end, even if narrow.

### Must Have (V1)

- [ ] **Organization CRUD** — create a Organization with Initiatives
...
- [ ] **Task management** — full lifecycle with hierarchy (tasks trace to organization goal)
...
- [ ] **Cost tracking** — Agents report token usage, per-Agent/task/Organization visibility
- [ ] **Budget controls** — soft alerts + hard ceiling with auto-pause
- [ ] **Default agent** — basic Claude Code/Codex loop with Rudder skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **Rudder skill (SKILL.md)** — teaches agents to interact with the API
...
- Public job board / open organization features

---

## 11. Knowledge Base

**Anti-goal for core.** The knowledge base is not part of the Rudder core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things Rudder explicitly does **not** do:

- **Not an Agent runtime** — Rudder orchestrates, Agents run elsewhere
