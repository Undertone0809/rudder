...
3. In Rudder UI, go to `http://127.0.0.1:3100/CLA/organization/settings`.
...
- Confirm pairing mode is explicit:
  - required default: device auth enabled (`agentRuntimeConfig.disableDeviceAuth` false/absent) with persisted `agentRuntimeConfig.devicePrivateKeyPem`
...
curl -sS -H "Cookie: $RUDDER_COOKIE" "http://127.0.0.1:3100/api/agents/$AGENT_ID" | jq '{agentRuntimeType,agentRuntimeConfig:{url:.agentRuntimeConfig.url,tokenLen:(.agentRuntimeConfig.headers["x-openclaw-token"] // .agentRuntimeConfig.headers["x-openclaw-auth"] // "" | length),disableDeviceAuth:(.agentRuntimeConfig.disableDeviceAuth // false),hasDeviceKey:(.agentRuntimeConfig.devicePrivateKeyPem // "" | length > 0)}}'
```
- Expected: `agentRuntimeType=openclaw_gateway`, `tokenLen >= 16`, `hasDeviceKey=true`, and `disableDeviceAuth=false`.
...
- This is a separate approval from Rudder invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Rudder with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Rudder UI that new issue exists.

10. Watch logs during test (optional but helpful):
```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.rudder-openclaw.override.yml logs -f openclaw-gateway
```
