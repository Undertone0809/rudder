...
Rudder V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Rudder-shaped design.

Assumption for this document: Rudder is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Rudder objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Rudder.

The main conclusion is:

- Rudder should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Rudder should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Rudder should use multiple extension classes instead of one generic plugin bag:
...
If Rudder does this well, the examples you listed become straightforward:
...
Relevant Rudder files reviewed for current extension seams:

- [server/src/agent-runtimes/registry.ts](../../server/src/agent-runtimes/registry.ts)
- [ui/src/agent-runtimes/registry.ts](../../ui/src/agent-runtimes/registry.ts)
...
It is too dangerous for Rudder core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Rudder — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Rudder should absolutely do this.
...
Rudder should copy this discipline.
...
Rudder should adopt this style for plugin actions, automations, and UI schemas.
...
Rudder can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Rudder too.

## What Rudder Should Not Copy Directly
...
Rudder is a control plane for an operator-managed instance with company objects.
...
Rudder is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Rudder should not auto-load arbitrary code from a workspace repo like `.rudder/plugins` or project directories.
...
For Rudder, equivalent hooks into:
...
Rudder should not allow plugins to silently replace:
...
For Rudder it would be risky because it combines:
...
## Why Rudder Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Rudder |
|---|---|---|
...
That means Rudder should borrow the good ideas from `opencode` but use a stricter architecture.

## Rudder Already Has Useful Pre-Plugin Seams

Rudder has several extension-like seams already:

- server adapter registry: [server/src/agent-runtimes/registry.ts](../../server/src/agent-runtimes/registry.ts)
- UI adapter registry: [ui/src/agent-runtimes/registry.ts](../../ui/src/agent-runtimes/registry.ts)
...
This is good news.
Rudder does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Rudder Plugin Model
...
Rudder already has this pattern implicitly:
...
- `plugin` means integration code that talks to Rudder through a typed plugin protocol and capability model
...
- invoke explicit Rudder actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@rudder/plugin-sdk/ui`) with design tokens and common components.
...
Rudder should treat plugin installation as a global instance-level action.

Examples:

- install `@rudder/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Rudder objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Rudder already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Rudder too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@rudder/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@rudder/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Rudder instance.

A `create-rudder-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Rudder achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Rudder server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Rudder's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@rudder/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@rudder/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Rudder

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@rudder/plugin-sdk";

export default definePlugin({
  id: "@rudder/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Rudder entities
    });
    ...
    ctx.events.on("plugin.@rudder/plugin-git.push-detected", async (event) => {
    ...
    ctx.actions.register("resync", async ({ orgId }) => {
      // run sync logic
    });
  },
});
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@rudder/plugin-sdk/ui";

export function DashboardWidget({ context }: PluginWidgetProps) {
  const { data, loading, error } = usePluginData("sync-health", { orgId: context.orgId });
...
Any Rudder plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Rudder objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@rudder/plugin-linear`)
...
- reuse core tables when the data is clearly part of Rudder itself
...
If a concept is becoming part of Rudder's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Rudder itself
...
### 3. Scope plugin data to Rudder objects before adding custom schemas

A lot of plugin data naturally hangs off existing Rudder objects:
...
If Rudder eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@rudder/plugin-workspace-files`
...
- Main page: `/:orgPrefix/plugins/workspace-files`
- Project tab: `/:orgPrefix/projects/:projectId?tab=files`
- Optional issue tab: `/:orgPrefix/issues/:issueId?tab=files`
- Optional agent tab: `/:orgPrefix/agents/:agentId?tab=workspace`
...
Package idea: `@rudder/plugin-terminal`
...
- Main page: `/:orgPrefix/plugins/terminal`
- Project tab: `/:orgPrefix/projects/:projectId?tab=terminal`
- Optional agent tab: `/:orgPrefix/agents/:agentId?tab=terminal`
- Optional run tab: `/:orgPrefix/agents/:agentId/runs/:runId?tab=terminal`
...
Package idea: `@rudder/plugin-git`
...
- Main page: `/:orgPrefix/plugins/git`
- Project tab: `/:orgPrefix/projects/:projectId?tab=git`
- Optional issue tab: `/:orgPrefix/issues/:issueId?tab=git`
- Optional agent tab: `/:orgPrefix/agents/:agentId?tab=git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Rudder.
- Board reviews the diff after a run without leaving Rudder.
...
- optional `events.emit` (e.g. `plugin.@rudder/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@rudder/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@rudder/plugin-linear`

This plugin syncs Rudder work with Linear. It is useful for:

- importing backlog from Linear
- linking Rudder issues to Linear issues
...
- Main page: `/:orgPrefix/plugins/linear`
- Dashboard widget: `/:orgPrefix/dashboard`
- Optional issue tab: `/:orgPrefix/issues/:issueId?tab=linear`
- Optional project tab: `/:orgPrefix/projects/:projectId?tab=linear`
...
- Plugin settings:
  ...
  - status mapping between Rudder and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Rudder.
- Rudder issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@rudder/plugin-github-issues`

This plugin syncs Rudder issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Rudder issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Rudder task governance

### UX

- Settings page: `/settings/plugins/github-issues`
- Main page: `/:orgPrefix/plugins/github-issues`
- Dashboard widget: `/:orgPrefix/dashboard`
- Optional issue tab: `/:orgPrefix/issues/:issueId?tab=github`
- Optional project tab: `/:orgPrefix/projects/:projectId?tab=github`
...
- Plugin settings:
  ...
  - whether new Rudder issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Rudder issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Rudder.
- GitHub webhooks update status/comment state in Rudder.
- A PR is linked back to the Rudder issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@rudder/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@rudder/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Rudder. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Rudder issues from anomalous metrics

### UX

- Settings page: `/settings/plugins/grafana`
- Main page: `/:orgPrefix/plugins/grafana`
- Dashboard widgets: `/:orgPrefix/dashboard`
- Optional goal tab: `/:orgPrefix/goals/:goalId?tab=metrics`
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Rudder plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Rudder dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Rudder issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Rudder core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@rudder/plugin-runtime-processes`
...
- Main page: `/:orgPrefix/plugins/runtime-processes`
- Dashboard widget: `/:orgPrefix/dashboard`
- Process detail page: `/:orgPrefix/plugins/runtime-processes/:processId`
- Project tab: `/:orgPrefix/projects/:projectId?tab=processes`
- Optional agent tab: `/:orgPrefix/agents/:agentId?tab=processes`
...
Package idea: `@rudder/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Rudder. It is useful for:
...
- Main page: `/:orgPrefix/plugins/stripe`
- Dashboard widgets: `/:orgPrefix/dashboard`
...
- Failed payment spikes or churn events can generate Rudder issues for follow-up.
...
- Stripe data should stay additive to Rudder core
...
- plugin UI bundle loading, host bridge, `@rudder/plugin-sdk/ui`
...
- `@rudder/plugin-test-harness` and `create-rudder-plugin` starter template
...
Rudder should not implement "an OpenCode-style generic in-process hook system."
Rudder should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Rudder
...
8. Ship `@rudder/plugin-test-harness` and `create-rudder-plugin` starter template.
