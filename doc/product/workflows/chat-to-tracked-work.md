...
3. A requested message-owned custom visual follows `CHAT.INLINE.VISUAL.001` and
   stays out of Library and the Chat Work manifest.
4. Rich references and issue/chat links remain readable through
   `CHAT.LIFECYCLE.001`, `LIBRARY.FILES.001`, and `ISSUE.COMMENTS.001`
   where applicable.
5. When the operator chooses structured coordination, create, link, or update
   issue work:
   `ISSUE.WORKFLOW.001`, `ISSUE.STATE.001`, `ISSUE.HIERARCHY.001`.
6. Assignment, reviewer, mention, and checkout paths use
   `ROUTING.ASSIGNMENT.001`, `ROUTING.REVIEWER.001`,
   `ROUTING.ATTENTION.001`, and `ROUTING.CHECKOUT.001`.
7. Messenger attention/read state uses `MESSENGER.ATTENTION.001`.
