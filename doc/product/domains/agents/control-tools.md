...
checkpoint label. While the one heterogeneous fallback remains,
`fallbackAction` identifies the alternate surface. For an MCP-origin item
read/comment failure it includes the exact equivalent `rudder issue ... --json`
command, including the Issue, comment, and supported query arguments available
from the request path. Comment writes materialize the original body into an
agent-owned temporary body file and preserve `--reopen`. For a CLI-origin
failure it names the equivalent typed MCP tool. Once the budget is consumed,
