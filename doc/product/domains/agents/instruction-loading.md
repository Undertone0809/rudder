...
- Wake context: `wakeReason`, `wakeSource`, `issue`, `comment`,
  ...
  fields when present.
- Goal Runtime Context: for `goal_started`, `goal_feedback`, and
  `goal_change_decided`, the accepted Contract, current persisted Plan,
  continuation, and wake-specific feedback or decision facts hydrated into the
  run snapshot.
...
- Goal Owner wakeups use one of three runtime-owned prompt templates:
  `GOAL_STARTED_PROMPT_TEMPLATE`, `GOAL_FEEDBACK_PROMPT_TEMPLATE`, or
  `GOAL_CHANGE_DECIDED_PROMPT_TEMPLATE`. Every Goal wake hydrates the current
  persisted Plan rather than assuming the activation Plan is still current.
...
9. For a Goal Owner wake, Rudder routes `goal_started`, `goal_feedback`, or
   `goal_change_decided` to its matching Goal prompt. Each template receives
   the same runtime boundary and nine-phase advancement protocol, plus an entry
   rule specific to the wake reason. The protocol advances the Goal as far as
   authority, evidence, and available tools allow in the current bounded Run;
   it is not a persisted workflow state machine.
...
- Every Goal wake snapshot hydrates the current persisted Plan and continuation.
- The current Agent-facing managed Goal tool set can read context, record
  progress, propose a Contract change, and propose a result. It does not yet
  expose a typed command for committing later Plan revisions, a typed Resume
  Condition, or optional Reviewer routing persistence.
- When those unsupported mechanisms are needed, the prompt must label the
  Plan/Replan, Wait, or review gate `Run-local and unpersisted`. It must not
  imply the state was saved, reviewed, scheduled, or will resume automatically.
...
- All three Goal Owner wake reasons use the shared nine-phase advancement
...
- The Goal service can persist Plan revisions, but the current Agent-facing
  managed Goal command surface cannot commit a Plan/Replan revision directly.
- Typed Resume Condition persistence and optional Goal Reviewer routing
  persistence are not implemented. Until they are, the Goal prompt must report
  those states as `Run-local and unpersisted`.
