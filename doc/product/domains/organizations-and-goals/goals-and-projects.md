...
flowchart TD
    ...
    Loop --> Checkpoint["Atomic Checkpoint: Plan + continuation + Evidence + activity"]
    Checkpoint -->|commitment / verification| ContinuationWake["goal_continuation wakes Owner"]
    ContinuationWake --> Loop
    ...
    Handoff -->|Feedback, decision, or named external wake| Loop
...
2. Every Goal wake hydrates the latest accepted Contract, current persisted Plan
   revision, latest checkpoint facts, and continuation into Goal Runtime Context.
   Wake-specific feedback, continuation, or change-decision facts are included
   when applicable.
...
6. The Owner records meaningful advancement through the atomic
   `rudder_goal_checkpoint` tool. It validates the current Plan revision, may
   persist the next Plan revision, updates the continuation, appends checkpoint
   and activity evidence, and queues at most one `goal_continuation` wake for
   `commitment` or `verification`. `wait` and `decision` persist the named
   handoff without automatically starting another Run.
...
- Implemented: canonical Goal Contract revisions; Owner assignments; initial
  ...
  requests and their hydrated context snapshots; append-only atomic Goal
  checkpoints with idempotency, Run attribution, and continuation wake routing.
- Implemented: the Agent-facing managed Goal tools can read Goal context,
  record progress, atomically checkpoint a Plan/continuation/evidence handoff,
  propose a Contract change, and propose a result.
- `goal_continuation` is an execution wake, not a new Goal lifecycle state. It
  carries the checkpoint, Plan revision, continuation, and context facts into
  the next bounded Run. Owner pause/unavailability uses the existing deferred
  wake behavior without losing the checkpoint.
- `wait` and `decision` remain durable handoffs but do not automatically wake a
  new Run. The prompt must name the actor, external condition, resume trigger,
  expiry, and safe fallback.
...
- `tests/e2e/goal-runtime-prompt.spec.ts` proves that `goal_started`,
  `goal_feedback`, `goal_change_decided`, and `goal_continuation` production
  wakeups reach adapter invocation with the current persisted Plan, checkpoint
  facts, and the complete Goal advancement protocol.
