# ClipHub — The Organization Registry
...
The tagline: **you can literally download a organization.**

---

## What Gets Published

A ClipHub package is a **organization template export** — the portable artifact format defined in the Rudder spec. It contains:
...
The homepage surfaces organizations across several dimensions:
...
- "marketing agency that runs facebook ads" → finds relevant organization templates even if those exact words aren't in the title
...
**Install (fresh start):**
```
rudder install cliphub:<publisher>/<organization-slug>
```
Downloads the template and creates a new organization in your local Rudder instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Rudder, export your organization as a template, then publish:

```
rudder export --template my-organization
rudder publish cliphub my-organization
```
...
| `description` | yes | What this organization does and who it's for |
...
Scans your local exported templates and publishes any that are new or updated. Useful for maintaining a portfolio of organization templates from a single repo.

---

## Categories

Organization templates are organized by use case:
...
Organization templates contain adapter configurations, which may include executable commands (process adapter) or webhook URLs (HTTP adapter). The moderation system:
...
ClipHub is a **separate service** from Rudder itself. Rudder is self-hosted; ClipHub is a hosted registry that Rudder instances talk to.
...
| **Rudder CLI** | `rudder install`, `rudder publish`, `rudder cliphub sync` — built into Rudder |
| **Rudder UI** | "Browse ClipHub" panel in the Rudder web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Rudder) |
| Backend | TypeScript + Hono (consistent with Rudder) |
| Database | PostgreSQL |
...
### "I want to start a organization"
...
1. Build and iterate on a organization in Rudder until it works well
2. Export: `rudder export --template my-agency`
3. Publish: `rudder publish cliphub my-agency`
4. Fill in description, category, tags on the web UI
5. Template is live — others can find and install it

### "I want to improve someone else's organization"
...
### "I just need one great agent, not a whole organization"
...
## Relationship to Rudder

ClipHub is **not required** to use Rudder. You can build organizations entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:

- **New users** get a working organization in minutes instead of hours
- **Experienced users** share proven configurations with the community
- **The ecosystem** compounds — every good template makes the next organization easier to build

ClipHub is to Rudder what a package registry is to a language runtime: optional, but transformative.
...
- [ ] "Browse ClipHub" panel in Rudder web UI
- [ ] `rudder cliphub sync` for bulk publishing
...
- Running organizations on ClipHub (it's a registry, not a runtime — consistent with Rudder's own philosophy)
