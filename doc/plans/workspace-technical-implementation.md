...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/rudder-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `RUDDER_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `RUDDER_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Rudder may create host-local worktrees, processes, and paths
- `cloud`
  - Rudder should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `rudder | github | vercel | s3 | custom`
...
- use `RUDDER_EXECUTION_TOPOLOGY` as a defaulting guardrail
