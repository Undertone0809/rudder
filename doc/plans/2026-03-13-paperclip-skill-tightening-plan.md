...
The `rudder` skill is part of the critical Rudder safety surface. Tightening it may reduce fresh-session token use, but it also carries prompt-regression risk. We do not yet have evals that would let us safely prove behavior preservation across assignment handling, checkout rules, comment etiquette, approval workflows, and escalation paths.
...
Fresh runs still spend substantial input tokens even after the context-path fixes. The remaining large startup cost appears to come from loading the full `rudder` skill and related instruction surface into context at run start.
...
- preserve all current Rudder capabilities
...
- prompt degradation on Rudder safety rules
...
- define acceptance scenarios for Rudder correctness
...
- materially lower first-run input tokens for Rudder-coordinated agents
