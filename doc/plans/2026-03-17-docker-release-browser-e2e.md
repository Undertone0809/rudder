...
Today release smoke testing for published Rudder packages is manual and shell-driven:
...
- builds `docker/Dockerfile.onboard-smoke`
- runs `rudder@${RUDDER_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@rudder.local`
- password: `rudder-smoke-password`
...
1. A published `rudder@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `rudder@latest` Docker install can be smoke-tested by Playwright in CI.
