...
This plan defines the concrete budgeting model Rudder should implement next.

## Product Goals

Rudder should let operators:
...
For the next implementation pass, Rudder should enforce these defaults:
...
Rudder should separate subscription-included usage from billed spend:
...
Codex subscription runs default to API-equivalent cost accounting through the
`countSubscriptionUsageAsCost` runtime config. When a Codex agent uses local
subscription auth and the switch is not explicitly disabled, Rudder records the
run as `metered_api` with estimated cents from the OpenAI/Codex token price
table, so known-model estimates consume Rudder spend and money-budget hard
stops. Unknown models remain subscription usage until a price is added. Turn the
switch off only for agents whose subscription usage should preserve the legacy
`subscription_included` `$0.00` behavior.
...
Rudder must know that a project is budget-paused and enforce it.
...
Rudder must also block execution before new work starts.
