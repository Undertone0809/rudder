...
Rudder's module system lets you extend the operating layer with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Rudder's API, UI, and data model | Yes |
...
modules/
  observability/
    rudder.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@rudderhq/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@rudderhq/core";
...
```
1. Scan modules/ directory for rudder.module.json manifests
...
```jsonc
// rudder.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@rudderhq/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@rudderhq/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:rudder/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.github.com/Undertone0809/rudder/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm rudder store list                    # browse available modules and templates
pnpm rudder store install <module-id>     # install a module
pnpm rudder store import <template-id>    # import a company template
pnpm rudder store export                  # export current company as template
```
...
| **Notifications** | Slack/email/webhook alerts on configurable triggers | All hooks (configurable) |
...
| **Multi-tenant** | Separate companies/orgs within one Rudder instance | `server:started` |
...
4. **Module config** — `rudder.config.json` with per-module config, env var interpolation
...
10. **`@rudderhq/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
