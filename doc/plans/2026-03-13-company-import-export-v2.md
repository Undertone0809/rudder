...
This document defines the next-stage plan for Rudder company import/export.

The core shift is:

- move from a Rudder-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Rudder.
...
Rudder already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Rudder
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `rudder.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.rudder.yaml` sidecar for high-fidelity Rudder-specific details
...
11. adapter-aware skill sync surfaces so Rudder can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Rudder at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Rudder sidecar.
...
Rudder must not redefine `SKILL.md`.
...
- Rudder-specific fidelity lives in `.rudder.yaml`
- Rudder may resolve and install `SKILL.md` packages, but it must not require a Rudder-only skill format
...
### 5.4 Base Package Vs Rudder Extension
...
- base package:
  ...
  - no Rudder-only runtime fields by default
- Rudder extension:
  - `.rudder.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Rudder tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`rudder.manifest.json` is not part of the future package direction.
...
Rudder import/export should support these entity kinds:
...
- Rudder can treat “this set of agents came from team package X” as the imported-team model
...
Rudder should support source references in package metadata with:
...
Rudder should define a new adapter surface area around skills:
...
Planned Rudder behavior:

- if an adapter supports read, Rudder should show current skills in the UI
- if an adapter supports write, Rudder should let the user enable/disable imported skills
- if an adapter supports sync, Rudder should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.rudder.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.rudder.yaml` alongside the base package
...
## 10. Storage Model Inside Rudder
...
Rudder should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `rudder company export <company-id> --out <path>`
- `rudder company import <path-or-url> --dry-run`
- `rudder company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Rudder storage, or only referenced at import time?
...
3. Should Rudder support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Rudder aligned with:
