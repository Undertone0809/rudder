...
4. Remove the broad always-injected prompt sentence that tells every run it can
   use the current `rudder` skill for nearly every control-plane object.
...
- domain coverage: major Rudder product and control-plane surfaces;
...
- `references/control-plane-practices.md`
...
| Workspace and Library explanation | Public docs or control-plane practices reference | Route by user-facing vs contributor question. |
...
- Remove the broad always-injected sentence recommending the current `rudder`
  skill for nearly every control-plane surface.
...
- Rename and rewrite
  ...
  `server/resources/bundled-skills/rudder-docs/SKILL.md`.
- Move and update its sibling references, adding the control-plane practices
