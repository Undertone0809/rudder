...
- Extend the first-party `rudder-tools` MCP/native bridge with high-level
...
- Only `http:` and `https:` navigation is allowed. `file:`, `javascript:`,
  `data:`, `devtools:`, Rudder origins, and arbitrary custom
...
Agent Browser tool
  -> rudder-tools MCP/native bridge
...
The tools reuse the existing first-party `rudder-tools` transport.
