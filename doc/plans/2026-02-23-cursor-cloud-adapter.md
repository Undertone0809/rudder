...
This document defines the V1 design for a Rudder adapter that integrates with
...
4. synthesized stdout events for Rudder UI/CLI
...
3. **Agent callback auth to Rudder** uses a bootstrap exchange flow (no long-lived Rudder key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Rudder endpoints, not full SKILL.md prompt injection.
...
```
packages/agent-runtimes/cursor-cloud/
├── package.json
├── tsconfig.json
└── src/
    ├── index.ts
...
- `paperclipPublicUrl` (optional override; else `RUDDER_PUBLIC_URL` env)
...
Use `agentRuntimeConfig.env` so secret references are supported by existing secret-resolution flow.

---

## Rudder Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `RUDDER_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `agentRuntimeConfig.rudderPublicUrl`
2. `process.env.RUDDER_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Rudder credentials in prompt text.

Flow:

1. Before launch/follow-up, Rudder mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Rudder validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Rudder API calls.
...
1. Prompt includes a compact instruction to fetch skills from Rudder.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/rudder`
   - `GET /api/skills/rudder-create-agent` when needed
...
4. Route by Cursor `agentId` to active Rudder run context.
...
### Adapter fields (`ui/src/agent-runtimes/cursor-cloud/config-fields.tsx`)
...
- `cli/src/agent-runtimes/registry.ts`

### Shared contract updates (required)

- add `cursor_cloud` to `packages/shared/src/constants.ts` (`AGENT_RUNTIME_TYPES`)
...
| Skill delivery | local skill dir injection | authenticated fetch from Rudder skill endpoints |
| Rudder auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Rudder context.
3. Add richer PR metadata surfacing in Rudder UI.
...
- [ ] `packages/agent-runtimes/cursor-cloud/package.json` exports wired
- [ ] `packages/agent-runtimes/cursor-cloud/tsconfig.json`
