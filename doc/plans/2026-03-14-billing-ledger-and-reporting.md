...
Rudder currently stores model spend in `cost_events` and operational run state in `heartbeat_runs`.
...
However, if Rudder needs to account for the full billing surface of aggregators and managed AI platforms, then `cost_events` alone is not enough.
...
It follows directly from the billing systems of providers and aggregators Rudder needs to support.
...
- add `executionAgentRuntimeType` to persisted cost reporting if adapter-level grouping becomes a product requirement
