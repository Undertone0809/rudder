...
`PAP-447` asks how Rudder should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Rudder instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Rudder:

- some operators use Rudder to manage Rudder and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Rudder
- Claude and Codex expose different built-in affordances, so Rudder should not overfit to one tool

## Core Product Decision

Rudder should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Rudder
...
- Rudder should not assume that every adapter can see or use a host filesystem path directly
...
If Rudder makes worktrees agent-first, it will blur:
...
- Rudder core resolves and records execution workspace state
...
That gives Rudder room to support:
...
- Rudder handles workspace runtime services under the hood using project/workspace policy
...
Once Rudder is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Rudder cannot infer
...
- `ui/src/agent-runtimes/local-workspace-runtime-fields.tsx`
- `ui/src/agent-runtimes/codex-local/config-fields.tsx`
- `ui/src/agent-runtimes/claude-local/config-fields.tsx`
...
- `packages/agent-runtimes/codex-local/src/ui/build-config.ts`
- `packages/agent-runtimes/claude-local/src/ui/build-config.ts`
...
- when issue moves to done, should Rudder auto-commit?
- should it auto-push?
...
Rudder already has the right foundation for a project-first model.
...
- Codex: `packages/agent-runtimes/codex-local/src/server/execute.ts`
- Claude: `packages/agent-runtimes/claude-local/src/server/execute.ts`
...
For server-spawned local adapters, Rudder already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Rudder should mirror directly

Implication:

- for `codex_local`, Rudder should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Rudder should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Rudder should separate:
...
Rudder should be able to express intentions such as:
...
These are not specific to Rudder. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".rudder/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Rudder-specific `devServer` flag.
...
Rudder should distinguish between:
...
Rudder should normalize the reported metadata without requiring every adapter to look like a host-local process.

Keep issue-level overrides possible through the existing `assigneeAgentRuntimeOverrides` shape in `packages/shared/src/types/issue.ts`.
...
In addition to the local git helper, Rudder should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Rudder
...
For runtime services specifically, Rudder should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Rudder-only dev-server flags
...
3. Inject resolved execution workspace details into `context.rudderWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Rudder instance can be reused through env/config injection when appropriate
...
- Rudder can decide whether to reuse or start a fresh service deterministically
...
3. Test against Rudder-on-Rudder workflows first.
...
7. Workspace runtime services are modeled generically, not as Rudder-specific dev-server toggles.
