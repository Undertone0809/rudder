...
This document defines the rollout plan for adapter-wide skill support in Rudder.
...
- Rudder stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Rudder currently has these adapters:
...
1. Desired skills live in Rudder for every adapter.
...
These adapters have a stable local skills directory that Rudder can read and manage.
...
These adapters do not have a meaningful Rudder-owned persistent install state.
...
- desired skills stored in Rudder
...
- runtime already injects Rudder skills into `~/.cursor/skills`
...
- runtime already injects Rudder skills into `~/.gemini/skills`
...
- runtime already injects Rudder skills into `~/.pi/agent/skills`
...
- Rudder can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Rudder skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Rudder may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Rudder skills that are clearly marked as Rudder-managed
...
- Rudder stores desired skills only
...
- avoid destructive wording unless Rudder can prove a skill is Rudder-managed
...
That gets Rudder from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
