...
- the adapter interface has no Rudder callback that can inject new
...
runtime-neutral Rudder, while Hermes, Codex, Claude, Gemini, OpenCode,
...
Rudder-owned events remain appropriate for decisions the model does not
make:
...
2. classify from the Rudder terminal reason plus validated adapter
...
- Adapters may contribute documentation descriptors and bounded
  ...
  Rudder validates hints and owns eligibility and budgets.
...
- `RUN.INTELLIGENCE.001`: expose bounded recovery evidence through run
