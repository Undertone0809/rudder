...
Add first-class **documents** to Rudder as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Rudder workflow/docs.
...
Update the Rudder agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/rudder/SKILL.md`
