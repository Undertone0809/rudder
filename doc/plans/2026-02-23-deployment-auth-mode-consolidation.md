...
Keep Rudder low-friction while making the mode model simpler and safer:
...
```sh
pnpm rudder configure --section server
```
...
1. `pnpm rudder onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm rudder doctor` works flagless with mode-aware checks.
