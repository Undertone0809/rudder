...
Add a single storage subsystem for Rudder that supports:
...
- API serves attachment bytes through authenticated Rudder endpoints.
...
- Default local path is under instance root: `~/.rudder/instances/<instanceId>/data/storage`.
...
- `rudder onboard` writes a valid `storage` config block by default.
- `rudder configure --section storage` can switch between local and s3 modes.
...
- `rudder doctor` reports actionable storage status.
