...
This document defines the product and UI plan for skill management in Rudder.
...
- Rudder company import/export can include skills as package content
...
3. The UI must always tell the truth about what Rudder knows:
   - desired state in Rudder
...
Rudder should treat skills at two scopes:
...
- created locally inside Rudder later
...
7. “Understand what Rudder knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Rudder-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Rudder did not install.

These should be clearly marked:

- external
- not managed by Rudder
...
- `desired_only`: Rudder wants it, adapter does not show it yet
- `external`: adapter has it but Rudder does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Rudder only tracks desired state
...
- Rudder can still track desired skills
...
- `.rudder.yaml` may add Rudder-specific fidelity, but should not replace the base shortname association model
...
- `.rudder.yaml` may add Rudder-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Rudder should have a company-scoped skill model or managed package model representing:
...
Rudder should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Rudder-specific.
...
That gives Rudder one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
