...
Agents need API keys to authenticate with Rudder. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Rudder
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Rudder generates a token and passes it directly to the agent
...
`RUDDER_API_URL`, `RUDDER_AGENT_ID`, `RUDDER_ORG_ID`. We need to
add a `RUDDER_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Rudder CLI
...
```
rudder auth login
# Opens browser -> user confirms -> token stored at ~/.rudder/credentials
```
...
managed by the Rudder server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Rudder.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Rudder SKILL.md (or a link to it)
   - What information Rudder needs from the agent (e.g. webhook URL, adapter
...
5. Rudder stores the pending registration.
...
7. On approval, Rudder provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Rudder issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.github.com/Undertone0809/rudder/skills/rudder/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "agentRuntimeType": "How Rudder should send you heartbeats",
...
1. Generate a Rudder invite link for the company.
...
4. A Rudder company member approves the new agent.
5. Rudder begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `RUDDER_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `rudder auth login` for developer-managed remote agents.                                      |
