...
Introduce a built-in Rudder core-tool surface for high-frequency Rudder
...
That works, but it creates a poor boundary for core Rudder actions:
...
- security:
  ...
  - plugin tools must not gain authority over checkout, approval, or other
    product invariants through this proposal
- maintainability:
...
2. The bundled `rudder` skill tells the agent to follow the heartbeat workflow
   and use Rudder core tools for standard Rudder actions.
...
Add a Rudder-specific semantic block so the operator sees Rudder work as
...
Prove that the common heartbeat high-traffic workflow path uses built-in Rudder tools,
