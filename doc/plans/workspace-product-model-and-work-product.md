...
Rudder needs to support two very different but equally valid ways of working:
...
Today, Rudder already has the beginnings of this model:
...
6. Seamlessly upgrade existing Rudder users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Rudder deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Rudder into a full code review product
...
This object must be recorded explicitly so that Rudder can:
...
Rudder should treat PRs as a type of work product linked back to:
...
Rudder already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Rudder must be a first-class deployment mode

Rudder cannot assume that it is running on the same machine as the code.

In cloud deployments, Rudder may:
...
- `Runtime service`: a process or service Rudder owns or tracks for a workspace
...
It also lets cloud-hosted Rudder deployments point at codebases and remotes without pretending the Rudder host has direct filesystem access.
...
Without an explicit `execution workspace` record, Rudder has nowhere stable to attach:
...
Rudder needs a single place to show:
...
- Rudder-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Rudder owning the execution host
...
## 1. Rudder host and execution host must be decoupled
...
without the Rudder host itself running local git or local preview processes.
...
Rudder should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Rudder.
...
Rudder should keep workspace automation centered on a small set of hooks:
...
Rudder should provide the equivalent visibility in the project `Code` surface:
...
Rudder should treat execution workspaces as the natural home for:
...
Rudder should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Rudder should continue treating cleanup policy as part of the core workspace model:
...
Rudder should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Rudder host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Rudder should borrow Worktrunk's good execution patterns while keeping the broader Rudder model:
...
That keeps Rudder focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Rudder deployments remain first-class

That is a better fit for Rudder than either extreme:
