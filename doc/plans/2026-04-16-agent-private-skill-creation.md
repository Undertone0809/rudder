...
Agent-private skills already exist as a runtime concept: Rudder discovers them from `AGENT_HOME/skills` and agents can enable them once they are present. The missing piece is the authored Rudder surface. There is no first-party create flow for agent-owned skill packages, so the product contract says "agent skills exist" while the mutation surface still behaves as if only organization skills are authorable.

## Diagnosis
- Primary layer: Rudder product/API gap
