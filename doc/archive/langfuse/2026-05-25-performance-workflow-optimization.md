...
Optimize Rudder's high-traffic high-traffic workflow paths without changing the current
...
Rudder's current high-traffic workflow paths often compute small summaries by loading
large object sets:
...
high-traffic workflow paths. `pnpm perf:workflows` now runs
`scripts/perf/workflow-baseline.ts` against an explicit `DATABASE_URL`,
...
- `DATABASE_URL=postgres://rudder:rudder@127.0.0.1:54339/<temp-db> pnpm perf:workflows -- --scale smoke --iterations 2`
- `pnpm --filter @rudderhq/server typecheck`
...
- `DATABASE_URL=postgres://rudder:rudder@127.0.0.1:54339/<temp-db> pnpm perf:workflows -- --scale smoke --iterations 1 --explain`
- `pnpm --filter @rudderhq/server typecheck`
- `git diff --check`
...
- `DATABASE_URL=postgres://rudder:rudder@127.0.0.1:54339/<temp-db> pnpm perf:workflows -- --scale medium --iterations 1 --explain`
...
- `pnpm --filter @rudderhq/server exec vitest run src/__tests__/messenger-service.test.ts --reporter=verbose`
  with `RUDDER_MESSENGER_SERVICE_TEST_DATABASE_URL` pointed at a temporary
  isolated database on the already-running local Rudder Postgres instance
- `DATABASE_URL=postgres://rudder:rudder@127.0.0.1:54339/<temp-db> pnpm perf:workflows -- --scale medium --iterations 2 --explain`
...
- `pnpm --filter @rudderhq/server exec vitest run src/__tests__/messenger-service.test.ts --reporter=verbose`
  ...
  isolated database on the already-running local Rudder Postgres instance
- `DATABASE_URL=postgres://rudder:rudder@127.0.0.1:54339/<temp-db> pnpm perf:workflows -- --scale medium --iterations 2 --explain`
- `DATABASE_URL=postgres://rudder:rudder@127.0.0.1:54339/<temp-db> pnpm perf:workflows -- --scale medium --iterations 5`
...
- `DATABASE_URL=postgres://rudder:rudder@127.0.0.1:54339/<temp-db> pnpm perf:workflows -- --scale cost-heavy --iterations 5 --explain`
- `DATABASE_URL=postgres://rudder:rudder@127.0.0.1:54339/<temp-db> pnpm perf:workflows -- --scale medium --iterations 5`
- `pnpm build`
