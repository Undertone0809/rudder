...
Rudder's UI is a professional-grade Rudder, not a toy dashboard. It should feel like the kind of tool you live in all day — fast, keyboard-driven, information-dense without being cluttered, dark-themed by default. Every pixel should earn its place.
...
```
┌─────────────────────────┐
│ [icon] Acme Corp      ▼ │  ← Organization switcher dropdown
├─────────────────────────┤
│ [🔍]  [✏️]              │  ← Search + New Issue
└─────────────────────────┘
```

**Organization switcher** is a dropdown button that occupies the full width of the sidebar header. It shows:
- Organization icon (first letter avatar with organization color, or uploaded icon)
- Organization name (truncated with ellipsis if long)
- Chevron-down icon

Clicking opens a dropdown with:
- List of all organizations (with status dot: green=active, yellow=paused, gray=archived)
- Search field at top of dropdown (for users with many organizations)
- Divider
- `+ Create organization` action at the bottom

Below the organization name, a row of icon buttons:
- **Search** (magnifying glass icon) — opens Cmd+K search modal
- **New Issue** (pencil/square-pen icon) — opens new issue modal in the current organization context

### 3.2 Personal Section

No section header — these are always at the top, below the organization header.
...
- **Issues** — main task list for the selected organization. This is the workhorse view.
- **Projects** — project list. Projects group issues and link to goals.
- **Goals** — organization goal hierarchy.
...
### 3.4 Organization Section

Section header: **Organization** (collapsible)
...
- **Dashboard** — organization health overview: agent statuses, task velocity, cost burn, pending approvals count.
- **Organization Structure** — interactive tree visualization of the agent reporting hierarchy.
...
Note: Approvals do not have a top-level sidebar entry. They are surfaced through the **Inbox** (primary interaction point), **Dashboard** (pending count metric), and **inline on entity pages** (e.g., an agent detail page shows the approval that authorized its hire). Full approval review happens inside Messenger at `/messenger/approvals`, with `/messenger/approvals/:approvalId` opening the approval as a modal state, not a separate page.
...
| Organization Structure | `GitBranch` |
| Agents | `Bot` |
| Costs | `DollarSign` |
| Activity | `History` |
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ Projects › Rudder › Issues › CLIP-42  [⭐] [···]     [🔔] [⬜] │
└─────────────────────────────────────────────────────────────────────┘
```
...
3. **Issue key** — e.g., `CLIP-5`. Monospace, muted color. The prefix is derived from the project (or organization if no project).
...
**Top bar:**
- Breadcrumb showing context: project key (or organization key) `›` "New issue".
...
├─────────────────────────────────────────────────────────┤
│ [icon] Rudder Auth     Backlog     CTO     Feb 20   │
...
│ Goals                                    [+ New goal]   │
├─────────────────────────────────────────────────────────┤
│ ▼ 🎯 Build the #1 AI note-taking app    Organization  Active│
...
Each row: expand chevron (if has children), target icon, title, level badge (Organization/Team/Agent/Task), status badge.
...
The dashboard is the organization health overview. Shown when clicking "Dashboard" in the Organization section.
...
**2. Dashboard metric card.** The "Pending Approvals" card shows the count and links to the Messenger approvals thread.
...
### 11.2 Approval Review Thread (`/messenger/approvals`)

This is the "See all" destination from Inbox and Dashboard. It is a Messenger thread, not a standalone page in the old board layout.
...
│ Approvals assistant                                  │
│ ┌───────────────────────────────────────────────────┐ │
│ │ Hire Agent: "Marketing Analyst"      pending     │ │
│ │ CEO · 2h ago                                     │ │
│ │ [Approve] [Reject] [Open full approval]          │ │
│ └───────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────┘
```

Each approval appears as an object card inside the thread: status, payload summary, requester, relative time, inline decision actions, and an "Open full approval" control that opens the approval modal.

### 11.3 Approval Detail View

Approval detail is rendered as a modal on top of the Messenger approvals thread. It shows the full payload, linked issues, comments, and decision actions without leaving the current context.

The modal renders the approval payload nicely based on type:
...
Top: organization-wide budget progress bar (large, prominent).
...
- "See all approvals →" link in the category header navigates to `/messenger/approvals`.

**Alerts.** Agent errors (failed heartbeats, error status) and budget alerts (agents or organization approaching 80% or 100% limits). Each links to the relevant agent or cost page.
...
- **No agents:** "No agents in this organization. Create an agent to start building your team." with a `[Create agent]` button.
- **No organization selected:** "Select a organization to get started." with a organization switcher or `[Create organization]` button.
...
All routes are organization-scoped after organization selection (organization context stored in React context, not URL):
...
/messenger/approvals        → approvals thread
/messenger/approvals/:approvalId → approval modal state
/costs                      → cost dashboard
/activity                   → activity log
/organizations                  → organization management (list/create)
...
1. Sidebar redesign (grouped sections, icons, organization switcher, badges)
...
### Phase 4: Organization-Level Views
