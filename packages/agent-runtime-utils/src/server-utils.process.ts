...
export const ISSUE_DOCUMENT_PROMPT_BODY_CHAR_LIMIT = 16_000;

export type IssueDocumentPromptInput = {
  planDocument?: {
    key?: string | null;
    title?: string | null;
    body?: string | null;
...
export function truncateIssueDocumentBody(body: string) {
...
export function formatDocumentHeading(key: string, title?: string | null) {
  const cleanTitle = typeof title === "string" ? title.trim() : "";
  return cleanTitle ? `### ${key} — ${cleanTitle}` : `### ${key}`;
}

export function readIssueDocumentPromptIssueId(input: IssueDocumentPromptInput) {
...
export function formatIssueDocumentReference(input: {
  issueId: string;
  key?: string | null;
  title?: string | null;
  latestRevisionNumber?: number | null;
