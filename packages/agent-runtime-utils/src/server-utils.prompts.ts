...
export const RUDDER_AGENT_OPERATING_CONTRACT = [
  ...
  "",
  "## Rudder Control-Plane Interface",
  "",
  "Prefer first-party Rudder MCP tools for normal Rudder control-plane work when this runtime exposes them. The managed server is `rudder-control-plane`, and tool names use the stable `rudder_<capability_id>` shape, for example `rudder_issue_context`, `rudder_issue_checkout`, `rudder_issue_comment`, `rudder_issue_done`, `rudder_issue_review`, `rudder_chat_send`, and `rudder_library_file_ref`.",
  "",
  "Use `rudder ... --json` only as a compatibility fallback when MCP is unavailable, a Rudder MCP tool returns a transport/configuration error, or the wake text explicitly says this run is in CLI or HTTP compatibility mode. Do not route ordinary issue, review, chat, run, approval, automation, skill, or Library control-plane actions through shell commands when the matching Rudder MCP tool is available.",
  "",
  "Do not provide `RUDDER_API_URL`, `RUDDER_API_KEY`, `RUDDER_ORG_ID`, `RUDDER_AGENT_ID`, or `RUDDER_RUN_ID` as tool arguments; Rudder-owned runtime environment supplies identity and auth. Do not use direct API calls or `curl` for normal control-plane work. Direct API access is only a diagnostic compatibility fallback after MCP and CLI fallback are blocked, and you must record the reason in the issue comment, chat reply, or run notes.",
  "",
  ...
  "",
  "When an issue comment, done comment, or blocker comment cites visual evidence from a local screenshot/image path, attach the image through the available Rudder control-plane interface instead of leaving only the filesystem path in the text. Prefer the matching MCP tool's image argument when available; use the Rudder CLI `--image <path>` option only as the compatibility fallback.",
  "",
