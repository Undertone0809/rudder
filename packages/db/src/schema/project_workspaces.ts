...
export const projectWorkspaces = pgTable(
  ...
  (table) => ({
    companyProjectIdx: index("project_workspaces_company_project_idx").on(table.orgId, table.projectId),
    ...
    companySharedKeyIdx: index("project_workspaces_company_shared_key_idx").on(table.orgId, table.sharedWorkspaceKey),
