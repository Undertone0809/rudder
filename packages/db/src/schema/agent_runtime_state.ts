...
export const agentRuntimeState = pgTable(
  ...
  (table) => ({
    companyAgentIdx: index("agent_runtime_state_company_agent_idx").on(table.orgId, table.agentId),
    companyUpdatedIdx: index("agent_runtime_state_company_updated_idx").on(table.orgId, table.updatedAt),
  }),
);
