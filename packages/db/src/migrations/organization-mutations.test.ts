...
async function receipt(
  ...
  return sql.unsafe(`INSERT INTO organization_mutation_receipts
    ...
    [org, key, "a".repeat(64), audit, JSON.stringify({ organization_id: org, version: 1, fence_epoch: 0 })]);
}
...
beforeAll(async () => {
  ...
  instance = new EmbeddedPostgres({
    databaseDir: path.join(root, "postgres"), user: "rudder", password: "rudder", port,
  ...
  // Upgrade real prior schema with existing rows, not a hand-written table mock.
  ...
  journal.entries = journal.entries.filter((entry: { idx: number }) => entry.idx <= 162);
...
describe("D1 durable mutation schema on real PostgreSQL", () => {
  it("upgrades without activating Rust or changing old Node organization writes", async () => {
    expect(await db`SELECT name FROM organizations WHERE id = ${originalOrg}`)
      .toMatchObject([{ name: "Before D1" }]);
    expect(await db`SELECT * FROM organization_mutation_state WHERE org_id = ${originalOrg}`).toHaveLength(0);
    await db`UPDATE organizations SET name = 'Node still writes' WHERE id = ${originalOrg}`;
    const org = await organization();
    const [row] = await db`SELECT owner, mutation_version::text, fence_epoch::text
      FROM organization_mutation_state WHERE org_id = ${org}`;
    expect(row).toEqual({ owner: "node", mutation_version: "0", fence_epoch: "0" });
  });
  ...
  it("does not grant Rust authority with an unfenced initial row", async () => {
    await expect(db`INSERT INTO organization_mutation_state (org_id, owner)
      VALUES (${originalOrg}, 'rust')`).rejects.toMatchObject({ code: "23514" });
    expect(await db`SELECT * FROM organization_mutation_state WHERE org_id = ${originalOrg}`).toHaveLength(0);
  });
  ...
  it("requires the receipt activity to belong to the same organization", async () => {
    ...
    await expect(db.begin((tx) => receipt(tx, own, audit))).rejects.toMatchObject({ code: "23503" });
    expect(await db`SELECT * FROM organization_mutation_receipts WHERE org_id = ${own}`).toHaveLength(0);
  });
  ...
  it("scopes durable keys to organization and never overwrites a conflicting receipt", async () => {
    ...
    await expect(receipt(db, own, await activity(own))).rejects.toMatchObject({ code: "23505" });
    const other = await organization();
    await receipt(db, other, await activity(other));
    expect(await db`SELECT * FROM organization_mutation_receipts WHERE org_id = ${own}`).toHaveLength(1);
  });
  ...
  it("preserves the old activity-first organization deletion transaction", async () => {
    ...
    expect(await db`SELECT * FROM organization_mutation_receipts WHERE org_id = ${org}`).toHaveLength(0);
    expect(await db`SELECT * FROM organization_mutation_state WHERE org_id = ${org}`).toHaveLength(0);
  });
  ...
  it("cannot delete an activity while its live organization receipt survives", async () => {
    ...
    await expect(db`DELETE FROM activity_log WHERE org_id = ${org} AND id = ${audit}`)
  ...
  it("rejects backwards versions and unfenced owner changes", async () => {
    ...
    await expect(db`UPDATE organization_mutation_state SET mutation_version = 1 WHERE org_id = ${org}`)
      .rejects.toMatchObject({ code: "23514" });
    await expect(db`UPDATE organization_mutation_state SET owner = 'rust' WHERE org_id = ${org}`)
      .rejects.toMatchObject({ code: "23514" });
    await db`UPDATE organization_mutation_state SET owner = 'rust', fence_epoch = 1 WHERE org_id = ${org}`;
    await expect(db`UPDATE organization_mutation_state SET fence_epoch = 0 WHERE org_id = ${org}`)
      .rejects.toMatchObject({ code: "23514" });
  });
  ...
  it("keeps receipts immutable and prevents resetting live authority rows", async () => {
    const org = await organization();
    await receipt(db, org, await activity(org));
    await expect(db`UPDATE organization_mutation_receipts SET command_fingerprint = ${"b".repeat(64)} WHERE org_id = ${org}`)
      .rejects.toMatchObject({ code: "23514" });
    await expect(db`DELETE FROM organization_mutation_state WHERE org_id = ${org}`)
      .rejects.toMatchObject({ code: "23514" });
    await expect(db`DELETE FROM organization_mutation_receipts WHERE org_id = ${org}`)
      .rejects.toMatchObject({ code: "23514" });
  });
  ...
  it("supports signed BIGINT bounds and rejects invalid receipt envelopes", async () => {
    ...
    const [state] = await db`SELECT mutation_version::text FROM organization_mutation_state WHERE org_id = ${org}`;
    expect(state.mutation_version).toBe("9223372036854775807");
    const audit = await activity(org);
    await expect(receipt(db, org, audit, "")).rejects.toMatchObject({ code: "23514" });
    await expect(receipt(db, org, audit, "x".repeat(257))).rejects.toMatchObject({ code: "23514" });
    await expect(db`INSERT INTO organization_mutation_receipts
      ...
      VALUES (${org}, 'bad-result', 'organization_branding', ${"a".repeat(64)}, 'applied', 1, 0, ${audit}, '{}'::jsonb)`)
      .rejects.toMatchObject({ code: "23514" });
  });
  ...
  it("rolls business and activity writes back when deferred receipt validation fails", async () => {
    ...
    await expect(db.begin(async (tx) => {
      await tx.unsafe("UPDATE organizations SET name = 'Must roll back' WHERE id = $1", [own]);
      await tx.unsafe(`INSERT INTO activity_log (id, org_id, actor_id, action, entity_type, entity_id)
        VALUES ($1, $2::uuid, 'synthetic-board', 'organization.updated', 'organization', $2::text)`, [localAudit, own]);
      await receipt(tx, own, foreignAudit);
    })).rejects.toMatchObject({ code: "23503" });
    expect(await db`SELECT name FROM organizations WHERE id = ${own}`)
      .toMatchObject([{ name: "Synthetic D1 organization" }]);
    expect(await db`SELECT id FROM activity_log WHERE org_id = ${own} AND id = ${localAudit}`).toHaveLength(0);
  });
});
