...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "rudder-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveRudderHomeDir(), "instances", resolveRudderInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".rudder", CONFIG_BASENAME);
...
function resolveRudderConfigPath(): string {
  if (process.env.RUDDER_CONFIG?.trim()) {
    return path.resolve(process.env.RUDDER_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveRudderEnvPath(configPath: string): string {
...
function parsePositiveIntFromEnv(value: string | undefined): number | null {
  if (!value) return null;
  return asPositiveInt(Number(value));
}
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveRudderEnvPath(configPath);
  ...
  const port =
    parsePositiveIntFromEnv(process.env.RUDDER_EMBEDDED_POSTGRES_PORT)
