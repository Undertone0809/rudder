...
type GoalRecord = { id: string; title: string; lifecycle: string };
...
function isKitchenSinkDemoCompany(organization: CompanyRecord): boolean {
  return organization.name.startsWith("Kitchen Sink Demo");
}
...
function usePluginOverview(orgId: string | null) {
  return usePluginData<OverviewData>("overview", orgId ? { orgId } : {});
}
...
function CompactSurfaceSummary({ label, entityType }: { label: string; entityType?: string | null }) {
  ...
  const entityQuery = usePluginData(
    "entity-context",
    orgId && entityId && resolvedEntityType
      ? { orgId, entityId, entityType: resolvedEntityType }
      : {},
  );
  ...
  return (
    <div style={layoutStack}>
      ...
      <button
        ...
        onClick={() => {
          if (!orgId) return;
          void writeMetric({ name: "surface_click", value: 1, orgId }).catch(console.error);
        }}
      >
        Record demo metric
      </button>
...
function KitchenSinkPageWidgets({ context }: { context: PluginPageProps["context"] }) {
  const overview = usePluginOverview(context.orgId);
    ...
    <div style={widgetGridStyle}>
      <MiniWidget title="Runtime Summary" eyebrow="Overview">
        <div style={{ display: "grid", gap: "4px", fontSize: "12px" }}>
          <div>Organizations: {overview.data?.counts.organizations ?? 0}</div>
        ...
        <div style={rowStyle}>
          <button
            ...
            onClick={() => {
              ...
              void emitDemoEvent({ orgId: context.orgId, message: "Triggered from Kitchen Sink page" })
                .then((next) => {
                  overview.refresh();
          ...
          <button
            ...
            onClick={() => {
              if (!context.orgId) return;
              void startProgressStream({ orgId: context.orgId, steps: 4 })
                .then(() => {
                  setQuickActionStatus({
          ...
          <button
            ...
            onClick={() => {
              if (!context.orgId) return;
              void writeMetric({ orgId: context.orgId, name: "page_quick_action", value: 1 })
                .then((next) => {
                  overview.refresh();
...
function KitchenSinkIssueCrudDemo({ context }: { context: PluginPageProps["context"] }) {
  ...
  async function loadIssues() {
    ...
    setLoading(true);
    try {
      const result = await hostFetchJson<HostIssueRecord[]>(`/api/organizations/${context.orgId}/issues`);
  ...
  async function handleCreate() {
    if (!context.orgId || !createTitle.trim()) return;
    try {
      await hostFetchJson(`/api/organizations/${context.orgId}/issues`, {
        method: "POST",
        body: JSON.stringify({
  ...
  return (
    <Section title="Issue CRUD">
      <div style={mutedTextStyle}>
        This is a regular embedded React page inside Rudder calling the board API directly. It creates, updates, and deletes issues for the current organization.
      </div>
      {!context.orgId ? (
        <div style={mutedTextStyle}>Select a organization to use issue demos.</div>
      ) : (
        <>
          ...
          <div style={{ display: "grid", gap: "10px" }}>
            ...
            {!loading && issues.length === 0 ? <div style={mutedTextStyle}>No issues yet for this organization.</div> : null}
          </div>
        </>
      )}
    </Section>
...
function KitchenSinkCompanyCrudDemo({ context }: { context: PluginPageProps["context"] }) {
  const toast = usePluginToast();
  const [organizations, setCompanies] = useState<CompanyRecord[]>([]);
  ...
  async function loadCompanies() {
    setLoading(true);
    try {
      const result = await hostFetchJson<Array<CompanyRecord & { status?: string }>>("/api/organizations");
      setCompanies(result);
      setDrafts(
        Object.fromEntries(
          result.map((organization) => [organization.id, { name: organization.name, status: organization.status ?? "active" }]),
        ),
      );
      setError(null);
    } catch (nextError) {
  ...
  async function handleCreate() {
    ...
    try {
      await hostFetchJson("/api/organizations", {
        method: "POST",
        body: JSON.stringify({
          name,
      ...
      toast({ title: "Demo organization created", body: name, tone: "success" });
      setNewCompanyName(`Kitchen Sink Demo ${Date.now()}`);
      await loadCompanies();
    } catch (nextError) {
      toast({ title: "Organization create failed", body: getErrorMessage(nextError), tone: "error" });
    }
  }
  ...
  async function handleSave(orgId: string) {
    ...
    if (!draft) return;
    try {
      await hostFetchJson(`/api/organizations/${orgId}`, {
        method: "PATCH",
        body: JSON.stringify({
      ...
      toast({ title: "Organization updated", body: draft.name.trim(), tone: "success" });
      await loadCompanies();
    } catch (nextError) {
      toast({ title: "Organization update failed", body: getErrorMessage(nextError), tone: "error" });
    }
  }

  async function handleDelete(organization: CompanyRecord) {
    try {
      await hostFetchJson(`/api/organizations/${organization.id}`, { method: "DELETE" });
      toast({ title: "Demo organization deleted", body: organization.name, tone: "info" });
      await loadCompanies();
    } catch (nextError) {
      toast({ title: "Organization delete failed", body: getErrorMessage(nextError), tone: "error" });
    }
  }

  const currentCompany = organizations.find((organization) => organization.id === context.orgId) ?? null;
  const demoCompanies = organizations.filter(isKitchenSinkDemoCompany);

  return (
    <Section title="Organization CRUD">
      <div style={mutedTextStyle}>
        The worker SDK currently exposes organization reads. This page shows a pragmatic embedded-app pattern for broader board actions by calling the host REST API directly.
      </div>
      <div style={subtleCardStyle}>
        <div style={rowStyle}>
        ...
        <div style={{ fontSize: "12px" }}>{currentCompany?.name ?? "No current organization selected"}</div>
      </div>
      <div style={{ display: "grid", gap: "10px", gridTemplateColumns: "minmax(0, 1fr) auto" }}>
        <input
          ...
          placeholder="Kitchen Sink Demo Organization"
        />
        <button type="button" style={primaryButtonStyle} onClick={() => void handleCreate()}>
          Create demo organization
        </button>
      </div>
      {loading ? <div style={mutedTextStyle}>Loading organizations…</div> : null}
      {error ? <div style={{ ...mutedTextStyle, color: "var(--destructive, #dc2626)" }}>{error}</div> : null}
      <div style={{ display: "grid", gap: "10px" }}>
        {demoCompanies.map((organization) => {
          const draft = drafts[organization.id] ?? { name: organization.name, status: "active" };
          const isCurrent = organization.id === context.orgId;
          return (
            <div key={organization.id} style={subtleCardStyle}>
              <div style={{ display: "grid", gap: "10px", gridTemplateColumns: "minmax(0, 1.5fr) 120px auto auto" }}>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setDrafts((current) => ({
                      ...current,
                      [organization.id]: { ...draft, status: event.target.value },
                    }))}
                >
                ...
                <button type="button" style={buttonStyle} onClick={() => void handleSave(organization.id)}>
                  Save
                </button>
                <button type="button" style={buttonStyle} onClick={() => void handleDelete(organization)} disabled={isCurrent}>
                  Delete
                </button>
              </div>
              {isCurrent ? <div style={{ ...mutedTextStyle, marginTop: "8px" }}>Current organization cannot be deleted from this demo.</div> : null}
            </div>
          );
        })}
        {!loading && demoCompanies.length === 0 ? (
          <div style={mutedTextStyle}>No demo organizations yet. Create one above and manage it from this page.</div>
        ) : null}
      </div>
    </Section>
  );
    ...
    >
      <Section title="Embedded App Demo">
        <div style={{ fontSize: "13px", lineHeight: 1.5 }}>
          Plugins can host their own React page and behave like a native organization page. Kitchen Sink now uses this route as a practical demo app, then keeps the lower-level worker console below for the rest of the SDK surface.
        </div>
      </Section>
      <div style={{ display: "grid", gap: "14px" }}>
        <Section title="Plugin Page Route">
          <div style={mutedTextStyle}>
            The organization sidebar entry opens this route directly, so the plugin feels like a first-class organization page instead of a settings subpage.
          </div>
          <a href={pluginPagePath(context.companyPrefix)} style={{ fontSize: "12px" }}>
            {pluginPagePath(context.companyPrefix)}
          </a>
        </Section>
        <Section title="Rudder Animation">
          <div style={mutedTextStyle}>
            This is the same Rudder ASCII treatment used in onboarding, copied into the example plugin so the package stays self-contained.
          </div>
          <AsciiArtAnimation />
        </Section>
      </div>
...
function KitchenSinkStorageDemo({ context }: { context: PluginPageProps["context"] }) {
  ...
  const revenueState = usePluginData<StateValueData>(
    "state-value",
    context.orgId
      ? { scopeKind: "organization", scopeId: context.orgId, stateKey }
      : {},
  );
  ...
  return (
    <Section title="Plugin Storage">
      <div style={mutedTextStyle}>
        This clicker persists into plugin-scoped organization storage. A real revenue plugin could store counters, sync cursors, or cached external IDs the same way.
      </div>
      {!context.orgId ? (
        <div style={mutedTextStyle}>Select a organization to use organization-scoped plugin storage.</div>
      ) : (
        <>
          <div style={{ display: "grid", gap: "4px" }}>
            <div style={{ fontSize: "26px", fontWeight: 700 }}>{currentValue}</div>
            <div style={mutedTextStyle}>Stored at `organization/{context.orgId}/{stateKey}`</div>
          </div>
          <div style={rowStyle}>
            {[-10, -1, 1, 10].map((delta) => (
          ...
          <JsonBlock value={revenueState.data ?? { scopeKind: "organization", stateKey, value: 0 }} />
        </>
      )}
    </Section>
  );
...
function KitchenSinkHostIntegrationDemo({ context }: { context: PluginPageProps["context"] }) {
  ...
  async function loadRuns() {
    ...
    try {
      const [nextLiveRuns, nextRecentRuns] = await Promise.all([
        hostFetchJson<HostLiveRunRecord[]>(`/api/organizations/${context.orgId}/live-runs?minCount=5`),
        hostFetchJson<HostHeartbeatRunRecord[]>(`/api/organizations/${context.orgId}/heartbeat-runs?limit=5`),
      ]);
  ...
  return (
    <Section title="Host Integrations">
      <div style={mutedTextStyle}>
        Plugin pages can feel like native Rudder pages. This section demonstrates host toasts, organization-scoped routing, and reading live heartbeat data from the embedded page.
      </div>
      <div style={subtleCardStyle}>
        <div style={rowStyle}>
          <strong>Organization Route</strong>
          <Pill label={pluginPagePath(context.companyPrefix)} />
        </div>
        <div style={mutedTextStyle}>
          This page is mounted as a real organization route instead of living only under `/plugins/:pluginId`.
        </div>
      </div>
      {!context.orgId ? (
        <div style={mutedTextStyle}>Select a organization to read run data.</div>
      ) : (
...
function KitchenSinkConsole({ context }: { context: { orgId: string | null; companyPrefix?: string | null; projectId?: string | null; entityId?: string | null; entityType?: string | null } }) {
  const orgId = context.orgId;
  const overview = usePluginOverview(orgId);
  ...
  const organizations = usePluginData<CompanyRecord[]>("organizations", { limit: companiesLimit });
  const projects = usePluginData<ProjectRecord[]>("projects", orgId ? { orgId, limit: projectsLimit } : {});
  const issues = usePluginData<IssueRecord[]>("issues", orgId ? { orgId, limit: issuesLimit } : {});
  const goals = usePluginData<GoalRecord[]>("goals", orgId ? { orgId, limit: goalsLimit } : {});
  const agents = usePluginData<AgentRecord[]>("agents", orgId ? { orgId } : {});
  ...
  const workspaceQuery = usePluginData<Array<{ id: string; name: string; path: string }>>(
    "workspaces",
    orgId && selectedProjectId ? { orgId, projectId: selectedProjectId } : {},
  );
  ...
  async function executeTool(name: string) {
    if (!orgId || !selectedAgentId || !projectRef) {
      setToolOutput({ error: "Select a organization, project, and agent first." });
      return;
    }
    try {
    ...
    <div style={{ display: "grid", gap: "14px" }}>
      ...
      >
        <div style={rowStyle}>
          ...
          <Pill label={`Organization: ${orgId ?? "none"}`} />
        ...
        {overview.data ? (
          <>
            <div style={{ display: "grid", gap: "8px", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))" }}>
              <StatusLine label="Organizations" value={overview.data.counts.organizations} />
      ...
      <Section title="Rudder Domain APIs">
        <div style={{ display: "grid", gap: "12px", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
          <PaginatedDomainCard
            title="Organizations"
            items={organizations.data ?? []}
            totalCount={overview.data?.counts.organizations ?? null}
            empty="No organizations."
            onLoadMore={() => setCompaniesLimit((current) => current + 20)}
            render={(item) => {
              const organization = item as CompanyRecord;
              return <div>{organization.name} <span style={{ opacity: 0.6 }}>({organization.id.slice(0, 8)})</span></div>;
            }}
          />
          <PaginatedDomainCard
            title="Projects"
          ...
          <PaginatedDomainCard
            ...
            render={(item) => {
              ...
              return <div>{goal.title} <span style={{ opacity: 0.6 }}>({goal.lifecycle})</span></div>;
            }}
          />
        </div>
      </Section>
        ...
        <div style={{ display: "grid", gap: "10px", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
          <form
            ...
            onSubmit={(event) => {
              ...
              if (!orgId) return;
              void createIssue({ orgId, projectId: selectedProjectId || undefined, title: issueTitle })
                .then((next) => {
                  setResult(next);
                  return refreshAll();
                })
          ...
          >
            ...
            <button type="submit" style={primaryButtonStyle} disabled={!orgId}>Create issue</button>
          </form>
          <form
            style={layoutStack}
            onSubmit={(event) => {
              event.preventDefault();
              if (!orgId || !selectedIssueId) return;
              void advanceIssueStatus({ orgId, issueId: selectedIssueId, status: "in_review" })
                .then((next) => {
                  setResult(next);
                  return refreshAll();
                })
          ...
          >
            ...
            <button type="submit" style={buttonStyle} disabled={!orgId || !selectedIssueId}>Move to in_review</button>
          </form>
          <form
            style={layoutStack}
            onSubmit={(event) => {
              event.preventDefault();
              if (!orgId) return;
              void createGoal({ orgId, title: goalTitle })
                .then((next) => {
                  setResult(next);
                  return refreshAll();
                })
      ...
      <Section title="Events + Streams">
        <div style={rowStyle}>
          <button
            ...
            onClick={() => {
              if (!orgId) return;
              void emitDemoEvent({ orgId, message: "Kitchen Sink manual event" })
                .then((next) => {
                  setResult(next);
                  overview.refresh();
                })
          ...
          <button
            ...
            onClick={() => {
              if (!orgId) return;
              void startProgressStream({ orgId, steps: 5 })
        ...
        <div style={{ display: "grid", gap: "12px", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
          ...
          <form
            ...
            onSubmit={(event) => {
              event.preventDefault();
              if (!orgId) return;
              void writeActivity({ orgId, entityType: context.entityType ?? undefined, entityId: context.entityId ?? undefined })
                .then((next) => setResult(next))
                .catch((error) => setResult({ error: error instanceof Error ? error.message : String(error) }));
            }}
          >
            ...
            <div style={rowStyle}>
              <button
                ...
                onClick={() => {
                  if (!orgId) return;
                  void writeMetric({ orgId, name: metricName, value: Number(metricValue || "1") })
                    .then((next) => setResult(next))
                    .catch((error) => setResult({ error: error instanceof Error ? error.message : String(error) }));
                }}
              >
              ...
              <button type="submit" style={buttonStyle} disabled={!orgId}>Write activity</button>
            </div>
          </form>
        </div>
      </Section>
      ...
      <Section title="Workspace + Process">
        <div style={{ display: "grid", gap: "10px", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
          ...
          >
            ...
            <div style={rowStyle}>
              <button type="submit" style={buttonStyle} disabled={!orgId || !selectedProjectId}>Write scratch file</button>
              <button
                type="button"
                style={buttonStyle}
                onClick={() => {
          ...
          >
            ...
            <button type="submit" style={buttonStyle} disabled={!orgId || !selectedProjectId}>Run command</button>
        ...
        <div style={{ display: "grid", gap: "12px", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
          <form
            ...
            onSubmit={(event) => {
              ...
              void invokeAgent({ orgId, agentId: selectedAgentId, prompt: "Kitchen Sink invoke demo" })
                .then((next) => setResult(next))
                .catch((error) => setResult({ error: error instanceof Error ? error.message : String(error) }));
            }}
          >
            ...
            <div style={rowStyle}>
              <button type="submit" style={primaryButtonStyle} disabled={!orgId || !selectedAgentId}>Invoke</button>
              <button
                type="button"
                style={buttonStyle}
                onClick={() => {
                  if (!orgId || !selectedAgentId) return;
                  void pauseAgent({ orgId, agentId: selectedAgentId })
                    .then((next) => {
                      setResult(next);
                      agents.refresh();
                    })
              ...
              <button
                ...
                onClick={() => {
                  if (!orgId || !selectedAgentId) return;
                  void resumeAgent({ orgId, agentId: selectedAgentId })
                    .then((next) => {
                      setResult(next);
                      agents.refresh();
                    })
          ...
          <form
            ...
            onSubmit={(event) => {
              event.preventDefault();
              if (!orgId || !selectedAgentId) return;
              void askAgent({ orgId, agentId: selectedAgentId, prompt: "Give a short greeting from the Kitchen Sink plugin." })
                .then((next) => setResult(next))
                .catch((error) => setResult({ error: error instanceof Error ? error.message : String(error) }));
            }}
          >
            <strong>Agent chat stream</strong>
            <button type="submit" style={buttonStyle} disabled={!orgId || !selectedAgentId}>Start chat demo</button>
    ...
    <form onSubmit={onSubmit} style={{ display: "grid", gap: "18px" }}>
      <div style={{ display: "grid", gap: "12px", gridTemplateColumns: "minmax(0, 1.8fr) minmax(220px, 1fr)" }}>
        <div style={{ display: "grid", gap: "8px" }}>
          <strong>About</strong>
          <div style={{ fontSize: "13px", lineHeight: 1.5 }}>
            Kitchen Sink demonstrates the current Rudder plugin API surface in one local, trusted example. It intentionally includes domain mutations, event handling, streams, tools, jobs, webhooks, and local workspace/process demos.
          </div>
          <div style={{ fontSize: "12px", opacity: 0.7 }}>
            Current organization context: {context.orgId ?? "none"}
          </div>
        </div>
        <div style={{ display: "grid", gap: "8px" }}>
...
export function KitchenSinkDashboardWidget({ context }: PluginWidgetProps) {
  ...
  return (
    <div style={layoutStack}>
      ...
      <div style={{ fontSize: "12px", opacity: 0.7 }}>
        Plugin runtime surface demo for the current organization.
      </div>
      <div style={{ display: "grid", gap: "4px", fontSize: "12px" }}>
      ...
      <div style={rowStyle}>
        ...
        <button
          ...
          onClick={() => {
            if (!context.orgId) return;
            void writeMetric({ orgId: context.orgId, name: "dashboard_click", value: 1 }).catch(console.error);
          }}
        >
          Write metric
        </button>
...
export function KitchenSinkToolbarButton() {
  ...
  return (
    <button
      ...
      onClick={() => {
        if (!context.orgId) return;
        void startProgress({ orgId: context.orgId, steps: 3 }).catch(console.error);
      }}
    >
      Kitchen Sink Action
    </button>
...
export function KitchenSinkCommentAnnotation({ context }: PluginCommentAnnotationProps) {
  ...
  const data = usePluginData<CommentContextData>(
    "comment-context",
    context.orgId
      ? { orgId: context.orgId, issueId: context.parentEntityId, commentId: context.entityId }
      : {},
  );
