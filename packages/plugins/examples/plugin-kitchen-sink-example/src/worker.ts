...
async function resolveWorkspace(
  ...
  const workspaces = await ctx.projects.listWorkspaces(projectId, orgId);
...
async function runCuratedCommand(
  ...
  const workspace = await resolveWorkspace(ctx, orgId, projectId, workspaceId);
...
async function listIssuesForCompany(ctx: PluginContext, orgId: string, limit = 50): Promise<Issue[]> {
  return await ctx.issues.list({ orgId, limit, offset: 0 });
}

async function listGoalsForCompany(ctx: PluginContext, orgId: string, limit = 50): Promise<Goal[]> {
  return await ctx.goals.list({ orgId, limit, offset: 0 });
}
...
async function registerDataHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.data.register("overview", async (params) => {
    ...
    const organizations = await ctx.organizations.list({ limit: 200, offset: 0 });
    const projects = orgId ? await ctx.projects.list({ orgId, limit: 200, offset: 0 }) : [];
    const issues = orgId ? await listIssuesForCompany(ctx, orgId, 200) : [];
    const goals = orgId ? await listGoalsForCompany(ctx, orgId, 200) : [];
    const agents = orgId ? await ctx.agents.list({ orgId, limit: 200, offset: 0 }) : [];
  ...
  ctx.data.register("organizations", async (params) => {
    return await ctx.organizations.list({ limit: getListLimit(params), offset: 0 });
  });

  ctx.data.register("projects", async (params) => {
    const orgId = getCurrentCompanyId(params);
    return await ctx.projects.list({ orgId, limit: getListLimit(params), offset: 0 });
  });

  ctx.data.register("issues", async (params) => {
    const orgId = getCurrentCompanyId(params);
    return await listIssuesForCompany(ctx, orgId, getListLimit(params));
  });

  ctx.data.register("goals", async (params) => {
    const orgId = getCurrentCompanyId(params);
    return await listGoalsForCompany(ctx, orgId, getListLimit(params));
  });

  ctx.data.register("agents", async (params) => {
    const orgId = getCurrentCompanyId(params);
    return await ctx.agents.list({ orgId, limit: getListLimit(params), offset: 0 });
  });
  ...
  ctx.data.register("workspaces", async (params) => {
    ...
    if (!projectId) return [];
    return await ctx.projects.listWorkspaces(projectId, orgId);
  });
  ...
  ctx.data.register("comment-context", async (params) => {
    ...
    const comments = await ctx.issues.listComments(issueId, orgId);
  ...
  ctx.data.register("entity-context", async (params) => {
    ...
    if (!orgId || !entityId || !entityType) return null;

    if (entityType === "project") {
      return await ctx.projects.get(entityId, orgId);
    }
    if (entityType === "issue") {
      return await ctx.issues.get(entityId, orgId);
    }
    if (entityType === "goal") {
      return await ctx.goals.get(entityId, orgId);
    }
    if (entityType === "agent") {
      return await ctx.agents.get(entityId, orgId);
    }
    return { entityId, entityType, orgId };
  });
}
...
async function registerActionHandlers(ctx: PluginContext): Promise<void> {
  ctx.actions.register("emit-demo-event", async (params) => {
    ...
    await ctx.events.emit("demo-event", orgId, {
      message,
      source: "kitchen-sink",
  ...
  ctx.actions.register("create-issue", async (params) => {
    ...
    const issue = await ctx.issues.create({ orgId, projectId, title, description });
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("advance-issue-status", async (params) => {
    ...
    const issue = await ctx.issues.update(issueId, { status: status as Issue["status"] }, orgId);
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("run-process", async (params) => {
    ...
    return await runCuratedCommand(ctx, config, orgId, projectId, workspaceId, commandKey);
  });
  ...
  ctx.actions.register("start-progress-stream", async (params) => {
    ...
    void (async () => {
      ctx.streams.open(STREAM_CHANNELS.progress, orgId);
      try {
  ...
  ctx.actions.register("invoke-agent", async (params) => {
    ...
    const result = await ctx.agents.invoke(agentId, orgId, { prompt, reason: "Kitchen Sink plugin demo" });
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("pause-agent", async (params) => {
    ...
    const agentId = typeof params.agentId === "string" ? params.agentId : "";
    if (!agentId) throw new Error("agentId is required");
    return await ctx.agents.pause(agentId, orgId);
  });
  ...
  ctx.actions.register("resume-agent", async (params) => {
    ...
    const agentId = typeof params.agentId === "string" ? params.agentId : "";
    if (!agentId) throw new Error("agentId is required");
    return await ctx.agents.resume(agentId, orgId);
  });
  ...
  ctx.actions.register("ask-agent", async (params) => {
    ...
    ctx.streams.open(STREAM_CHANNELS.agentChat, orgId);
    const session = await ctx.agents.sessions.create(agentId, orgId, {
      reason: "Kitchen Sink plugin chat demo",
    });

    await ctx.agents.sessions.sendMessage(session.sessionId, orgId, {
      prompt,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    ...
    {
      displayName: "Kitchen Sink Organization Summary",
      description: "Summarizes current organization counts from the Rudder APIs.",
      parametersSchema: { type: "object", properties: {} },
    },
    async (_params, runCtx): Promise<ToolResult> => {
      const projects = await ctx.projects.list({ orgId: runCtx.orgId, limit: 50, offset: 0 });
      const issues = await ctx.issues.list({ orgId: runCtx.orgId, limit: 50, offset: 0 });
      const goals = await ctx.goals.list({ orgId: runCtx.orgId, limit: 50, offset: 0 });
      const agents = await ctx.agents.list({ orgId: runCtx.orgId, limit: 50, offset: 0 });
      return {
        content: `Organization has ${projects.length} projects, ${issues.length} issues, ${goals.length} goals, and ${agents.length} agents.`,
        data: {
          orgId: runCtx.orgId,
          projects: projects.length,
          issues: issues.length,
