...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    ctx.data.register("comment-file-links", async (params: Record<string, unknown>) => {
      ...
      if (!commentId || !issueId || !orgId) return { links: [] };
      try {
    ...
    ctx.data.register(
      ...
      async (params: Record<string, unknown>) => {
        ...
        if (!projectId || !orgId || !workspaceId) return { entries: [] };
        const workspaces = await ctx.projects.listWorkspaces(projectId, orgId);
    ...
    ctx.data.register(
      ...
      async (params: Record<string, unknown>) => {
        ...
        if (!projectId || !orgId || !workspaceId || !filePath) {
    ...
    ctx.actions.register(
      ...
      async (params: Record<string, unknown>) => {
        ...
        if (!projectId || !orgId || !workspaceId) {
