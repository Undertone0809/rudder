...
export function FilesTab({ context }: PluginDetailTabProps) {
  ...
  const fileListParams = useMemo(
    () => (selectedWorkspace ? { projectId, orgId, workspaceId: selectedWorkspace.id } : {}),
    [orgId, projectId, selectedWorkspace],
  );
  ...
  const fileContentParams = useMemo(
    () =>
      selectedPath && selectedWorkspace
        ? { projectId, orgId, workspaceId: selectedWorkspace.id, filePath: selectedPath }
        : null,
    [orgId, projectId, selectedWorkspace, selectedPath],
  );
