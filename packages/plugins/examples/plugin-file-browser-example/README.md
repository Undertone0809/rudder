...
Example Rudder plugin that demonstrates:
...
- **getData `workspaces`** — `ctx.projects.listWorkspaces(projectId, orgId)` (ordered, primary first).
...
```bash
pnpm --filter @rudder/plugin-file-browser-example build
pnpm rudder plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm rudder plugin uninstall rudder-file-browser-example --force
```
...
- Optional: use `rudder-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
