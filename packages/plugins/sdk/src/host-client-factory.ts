...
import type { PluginCapability } from "@rudder/shared";
...
export interface HostServices {
  ...
  /** Provides `organizations.list`, `organizations.get`. */
  organizations: {
    list(params: WorkerToHostMethods["organizations.list"][0]): Promise<WorkerToHostMethods["organizations.list"][1]>;
    get(params: WorkerToHostMethods["organizations.get"][0]): Promise<WorkerToHostMethods["organizations.get"][1]>;
  };
...
export function createHostClientHandlers(
  ...
  return {
    ...
    // Companies
    "organizations.list": gated("organizations.list", async (params) => {
      return services.organizations.list(params);
    }),
    "organizations.get": gated("organizations.get", async (params) => {
      return services.organizations.get(params);
    }),
