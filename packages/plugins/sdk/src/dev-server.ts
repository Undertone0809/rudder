...
const DEV_SERVER_HEALTH_PATHS = new Set(["/__rudder__/health", "/__paperclip__/health"]);
const DEV_SERVER_EVENTS_PATHS = new Set(["/__rudder__/events", "/__paperclip__/events"]);
 ...
 *
 * Endpoint summary:
 * - `GET /__rudder__/health` for diagnostics
 * - `GET /__rudder__/events` for hot-reload stream
 * - Legacy `GET /__paperclip__/...` aliases remain supported
...
export async function startPluginDevServer(options: PluginDevServerOptions = {}): Promise<PluginDevServer> {
  ...
  const handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (DEV_SERVER_HEALTH_PATHS.has(url)) {
      sendJson(res, { ok: true, rootDir, uiDir });
      return;
    }

    if (DEV_SERVER_EVENTS_PATHS.has(url)) {
      res.writeHead(200, {
