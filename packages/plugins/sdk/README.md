...
Official TypeScript SDK for Rudder plugin authors.

- **Worker SDK:** `@rudder/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@rudder/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@rudder/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@rudder/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@rudder/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@rudder/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@rudder/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@rudder/plugin-sdk/ui/hooks` | Hooks only |
| `@rudder/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@rudder/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@rudder/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@rudder/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@rudder/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@rudder/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Rudder app. They can call ordinary Rudder HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Rudder instance at runtime.
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@rudder/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Rudder information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@rudder/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@rudder/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@rudder/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@rudder/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@rudder/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@rudder/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@rudder/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@rudder/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@rudder/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@rudder/plugin-sdk/ui";
...
```bash
rudder-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@rudder/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__rudder__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__rudder__/events` streams `reload` SSE events on UI build changes
- Legacy `GET /__paperclip__/...` aliases remain supported for compatibility
