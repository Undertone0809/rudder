...
const CODEX_BENIGN_STDERR_RES = [
  ...
  /^\d{4}-\d{2}-\d{2}T[^\s]+\s+WARN\s+codex_rmcp_client::stdio_server_launcher:\s+Failed to kill MCP process group for server rudder-operating-layer:\s+No such process\s+\(os error 3\)$/i,
