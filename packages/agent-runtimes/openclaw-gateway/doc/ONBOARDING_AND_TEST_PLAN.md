...
## Scope
This plan is now **gateway-only**. Rudder supports OpenClaw through `openclaw_gateway` only.
...
3. Device auth stays enabled by default; pairing is persisted via `agentRuntimeConfig.devicePrivateKeyPem`.
...
6. OpenClaw claims API key and installs/uses Rudder skill.
