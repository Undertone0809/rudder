...
function resolveRudderApiUrlOverride(value: unknown): string | null {
...
function buildRudderEnvForWake(ctx: AgentRuntimeExecutionContext, wakePayload: WakePayload): Record<string, string> {
  ...
  if (wakePayload.approvalStatus) rudderEnv.RUDDER_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    rudderEnv.RUDDER_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
export function buildWakeText(payload: WakePayload, rudderEnv: Record<string, string>): string {
  ...
  const lines = [
    ...
    ...envLines,
    `RUDDER_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load RUDDER_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "- Use Authorization: Bearer $RUDDER_API_KEY on every API call.",
    ...
    "   - For each linked issue, close resolved work or comment what remains; do not ignore issue state changed by the approval.",
    `3) Determine issueId: RUDDER_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "4) If RUDDER_WAKE_COMMENT_ID exists, read the issue and comments before acting:",
    "   - GET /api/issues/{issueId}",
    "   - GET /api/issues/{issueId}/comments",
    "   - A mention requests attention; it does not transfer ownership unless the comment explicitly says so.",
    "   - If the issue is not assigned to you, including user-owned or unassigned issues, and the comment does not explicitly ask you to implement, modify files, close the issue, or take ownership, respond to the comment itself. Do not checkout or execute the whole issue in that branch.",
    "5) If issueId exists and you own the issue, or the wake explicitly asks you to take ownership or execute issue work:",
    "   - GET /api/issues/{issueId} and GET /api/issues/{issueId}/comments unless already fetched in step 4.",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$RUDDER_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    "     If checkout returns 409, another agent owns the task; do not retry or keep working.",
    "   - Execute the issue instructions exactly.",
    "   - If instructions require a comment, POST /api/issues/{issueId}/comments with {\"body\":\"...\"}.",
    "   - For assignee completion, PATCH /api/issues/{issueId} with {\"status\":\"done\",\"comment\":\"what changed and why\"}.",
    "   - For blocked work, PATCH /api/issues/{issueId} with {\"status\":\"blocked\",\"comment\":\"exact blocker and next human/external action\"}.",
    "   - For reviewer close-out, PATCH /api/issues/{issueId} with {\"reviewDecision\":\"approve|request_changes|needs_followup|blocked\",\"comment\":\"structured review evidence\"}. Do not leave only a free-form accept/reject comment.",
    "6) If issueId does not exist:",
    "   - GET /api/orgs/$RUDDER_ORG_ID/issues?assigneeAgentId=$RUDDER_AGENT_ID&status=todo,in_progress,blocked",
    "   - GET /api/orgs/$RUDDER_ORG_ID/issues?reviewerAgentId=$RUDDER_AGENT_ID&status=in_review,blocked",
    "   - Prioritize reviewer rows in in_review or blocked, then assignee in_progress, then assignee todo. Skip blocked assignee work unless new context lets you unblock it.",
    ...
    "- GET /api/approvals/{approvalId}",
    "- GET /api/approvals/{approvalId}/issues",
    "- GET /api/orgs/{orgId}/issues?assigneeAgentId={agentId}&status=todo,in_progress,blocked",
    "- GET /api/orgs/{orgId}/issues?reviewerAgentId={agentId}&status=in_review,blocked",
...
function buildStandardRudderPayload(
  ...
  const workspaces = Array.isArray(workspaceEntries)
    ? workspaceEntries.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  ...
  const standardRudder: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: rudderEnv.RUDDER_API_URL ?? null,
  };

  if (workspace) {
    standardRudder.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardRudder.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardRudder.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "rudder-complete");
    this.ws = null;
  }
...
function parseUsage(value: unknown): AgentRuntimeExecutionResult["usage"] | undefined {
  const record = asRecord(value);
...
function extractRuntimeServicesFromMeta(meta: Record<string, unknown> | null): AgentRuntimeServiceReport[] {
  if (!meta) return [];
  const reports: AgentRuntimeServiceReport[] = [];
...
export async function execute(ctx: AgentRuntimeExecutionContext): Promise<AgentRuntimeExecutionResult> {
  ...
  const rudderEnv = buildRudderEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, rudderEnv);
  ...
  while (true) {
    ...
    try {
      ...
      const detailedMessage = pairingRequired
        ? `${message}. Approve the pending device in OpenClaw (for example: openclaw devices approve --latest --url <gateway-ws-url> --token <gateway-token>) and retry. Ensure this agent has a persisted agentRuntimeConfig.devicePrivateKeyPem so approvals are reused.`
        : message;
