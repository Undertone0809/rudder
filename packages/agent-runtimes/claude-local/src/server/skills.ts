...
export function resolveClaudeSkillsHome(config: Record<string, unknown>) {
  const env =
...
async function buildClaudeSkillSnapshot(config: Record<string, unknown>): Promise<AgentRuntimeSkillSnapshot> {
...
export async function listClaudeSkills(ctx: AgentRuntimeSkillContext): Promise<AgentRuntimeSkillSnapshot> {
