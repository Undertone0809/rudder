...
export function resolveManagedGeminiHomeDir(config: Record<string, unknown>, orgId: string) {
  const env =
...
export function resolveGeminiSkillsHome(config: Record<string, unknown>, orgId: string) {
  return path.join(resolveManagedGeminiHomeDir(config, orgId), ".gemini", "skills");
}

async function buildGeminiSkillSnapshot(
  ...
  const skillsHome = resolveGeminiSkillsHome(config, orgId);
...
export async function listGeminiSkills(ctx: AgentRuntimeSkillContext): Promise<AgentRuntimeSkillSnapshot> {
  return buildGeminiSkillSnapshot(ctx.config, ctx.orgId);
}
...
export async function syncGeminiSkills(
  ...
  const skillsHome = resolveGeminiSkillsHome(ctx.config, ctx.orgId);
  ...
  return buildGeminiSkillSnapshot(ctx.config, ctx.orgId, warnings);
}
