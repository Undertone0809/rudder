...
describe("OpenCode Browser capability", () => {
  it.each([
  ...
  ])("writes only the trusted run capability into managed environment ($expected)", async ({ enabled, expected, untrusted }) => {
    ...
    expect(config.environment?.RUDDER_BROWSER_ENABLED).toBe(expected);
  });
});
