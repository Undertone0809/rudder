...
describe("Hermes gateway execution", () => {
  ...
  it("resumes an automation issue session through its verified compression continuation", async () => {
    const server = await listen((req, res) => {
      if (req.url === "/api/sessions/hermes-session-root" && req.method === "GET") {
        return json(res, 200, {
          object: "hermes.session",
          session: { id: "hermes-session-root", end_reason: "compression" },
        });
      }
      if (req.url === "/api/sessions/hermes-session-root/messages" && req.method === "GET") {
        return json(res, 200, {
          object: "list",
          session_id: "hermes-session-tip",
          data: [{ role: "user" }],
        });
      }
      if (req.url === "/api/sessions/hermes-session-tip" && req.method === "GET") {
        return json(res, 200, {
          object: "hermes.session",
          session: { id: "hermes-session-tip", parent_session_id: "hermes-session-middle" },
        });
      }
      if (req.url === "/api/sessions/hermes-session-middle" && req.method === "GET") {
        return json(res, 200, {
          object: "hermes.session",
          session: {
            id: "hermes-session-middle",
            parent_session_id: "hermes-session-root",
            end_reason: "compression",
          },
        });
      }
      if (req.url === "/v1/runs" && req.method === "POST") {
        return json(res, 202, { run_id: "hermes-run-compressed", status: "started" });
      }
      if (req.url === "/v1/runs/hermes-run-compressed/events") {
        res.writeHead(200, { "content-type": "text/event-stream" });
        res.end(`data: ${JSON.stringify({ event: "run.completed", output: "resumed" })}\n\n`);
        return;
      }
      throw new Error(`unexpected ${req.method} ${req.url}`);
    });
    ...
    const result = await execute(context(
      { url: server.url, timeoutMs: 1_000 },
      {
        runtime: {
          ...
          sessionParams: {
            sessionId: "hermes-session-root",
            hermesSessionId: "hermes-session-root",
            cwd: "/canonical/agents/alex",
          },
          sessionDisplayId: "hermes-session-root",
          taskKey: "issue-hermes-1",
        },
        context: {
          issueId: "issue-hermes-1",
    ...
    expect(server.requests.map((request) => request.path)).toEqual([
      "/api/sessions/hermes-session-root",
      "/api/sessions/hermes-session-root/messages",
      "/api/sessions/hermes-session-tip",
      "/api/sessions/hermes-session-middle",
      "/api/sessions/hermes-session-middle",
      "/api/sessions/hermes-session-root",
      "/v1/runs",
      "/v1/runs/hermes-run-compressed/events",
    ]);
  });

  it("rejects a messages provider ID that is not a compression continuation", async () => {
    const server = await listen((req, res) => {
      if (req.url === "/api/sessions/hermes-session-root" && req.method === "GET") {
        return json(res, 200, {
          object: "hermes.session",
          session: { id: "hermes-session-root", end_reason: "completed" },
        });
      }
      if (req.url === "/api/sessions/hermes-session-root/messages" && req.method === "GET") {
        return json(res, 200, {
          object: "list",
          session_id: "hermes-session-fork",
          data: [],
        });
      }
      if (req.url === "/api/sessions/hermes-session-fork" && req.method === "GET") {
        return json(res, 200, {
          object: "hermes.session",
          session: { id: "hermes-session-fork", parent_session_id: "hermes-session-root" },
        });
      }
      throw new Error(`unexpected ${req.method} ${req.url}`);
    });
    ...
    const result = await execute(context(
      { url: server.url, timeoutMs: 1_000 },
      {
        runtime: {
          sessionId: "hermes-session-root",
          sessionParams: { sessionId: "hermes-session-root" },
          sessionDisplayId: "hermes-session-root",
          taskKey: "issue-hermes-1",
        },
      },
    ...
    expect(result).toMatchObject({
      ...
      errorMessage:
        "Hermes session messages returned provider ID \"hermes-session-fork\", " +
        "which is not a verified compression continuation of \"hermes-session-root\".",
    });
    expect(server.requests.some((request) => request.path === "/v1/runs")).toBe(false);
  });
