 ...
 *
 * Plugin bundles are built with `external: ["@rudderhq/plugin-sdk/ui", "react", "react-dom"]`,
 * so their ESM output contains bare specifier imports like:
 *
 * ```js
 * import { usePluginData } from "@rudderhq/plugin-sdk/ui";
 ...
 *    host's global bridge registry (`globalThis.__rudderPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__rudderPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__rudderPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@rudderhq/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@rudderhq/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@rudderhq/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@rudderhq/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@rudderhq/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__rudderPluginBridge__) {
 ...
 * self-register via `window.rudderPlugins.registerReactComponent()`.
 ...
 * of `@rudderhq/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
...
function slotContextToHostContext(
  ...
  return {
    orgId: pluginSlotContext.orgId ?? null,
    orgPrefix: pluginSlotContext.orgPrefix ?? null,
