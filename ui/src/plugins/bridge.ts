 ...
 * `useHostContext` from `@rudder/plugin-sdk/ui`.  Those are type-only
 ...
 * registry on `globalThis.__rudderPluginBridge__`), context propagation
...
export function usePluginData<T = unknown>(
  ...
  }, [pluginId, key, paramsKey, refreshCounter, orgId, renderEnvironmentKey]);
 ...
 *
 * Returns the current host context (organization, project, entity, user)
...
export function usePluginStream<T = unknown>(
  ...
  const { pluginId, hostContext } = usePluginBridgeContext();
  const effectiveCompanyId = options?.orgId ?? hostContext.orgId ?? undefined;
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ orgId: effectiveCompanyId });
