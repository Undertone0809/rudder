...
describe("translateMessage", () => {
  ...
  it("translates legacy hard-coded strings for zh-CN", () => {
    ...
    expect(translateLegacyString("zh-CN", "Read apple-mail.md")).toBe("读取 apple-mail.md");
    expect(translateLegacyString("zh-CN", "Read 2 files, used 2 tools")).toBe("读取了 2 个文件，使用了 2 个工具");
    expect(translateLegacyString("zh-CN", "Used 3 skills, ran 5 commands")).toBe("使用了 3 个技能，运行了 5 条命令");
    expect(translateLegacyString("zh-CN", "Outputs")).toBe("产出");
    expect(translateLegacyString("zh-CN", "Sources")).toBe("来源");
    expect(translateLegacyString("zh-CN", "References")).toBe("引用");
    expect(translateLegacyString("zh-CN", "Subagents")).toBe("子智能体");
    expect(translateLegacyString("zh-CN", "View all 12")).toBe("查看全部 12 项");
    expect(translateLegacyString("zh-CN", "Open subagents, 1 active · 2 done")).toBe("打开子智能体，1 个运行中 · 2 个已完成");
    expect(translateLegacyString("zh-CN", "Jump to source message for Brief.md")).toBe("跳转到 Brief.md 的来源消息");
