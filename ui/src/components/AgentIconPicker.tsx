...
import { AGENT_ICON_NAMES, type AgentIconName } from "@rudder/shared";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
export function AgentIcon({ icon, className }: AgentIconProps) {
  ...
  const isNamedIcon = normalized
    ? AGENT_ICON_NAMES.includes(normalized as AgentIconName)
    : false;
  if (normalized && !isNamedIcon) {
    return (
