...
import { filterChatAssistantTranscriptEntries, flattenChatTranscriptActions, getTranscriptMcpBrandIcon, segmentChatTranscriptBlocks, TranscriptChatToolActionRow } from "./RunTranscriptView.chat";
import { transcriptBlockIdentity, transcriptEntryIdentity, type TranscriptBlock } from "./RunTranscriptView.common";
import { coalesceRawTranscriptEntries } from "./RunTranscriptView.detail";
...
describe("RunTranscriptView", () => {
  ...
  it("keeps streaming transcript identities stable as delta timestamps advance", () => {
    const initial = normalizeChatTranscriptTurns([
      {
        kind: "assistant",
    ...
    const updated = normalizeChatTranscriptTurns([
      {
        kind: "assistant",
        ts: "2026-08-19T00:00:00.000Z",
        text: "First",
    ...
    expect(updated.turns[0]?.blocks[0]).toMatchObject({
      type: "message",
      text: "First streamed",
      ts: "2026-08-19T00:00:01.000Z",
    });
    expect(updated.turns[0]?.key).toBe(initial.turns[0]?.key);
    expect(segmentChatTranscriptBlocks(initial.turns[0]?.blocks ?? [])[0]?.key)
      .toBe(segmentChatTranscriptBlocks(updated.turns[0]?.blocks ?? [])[0]?.key);
  });

  it("keeps streamed stdout action identity stable as output grows", () => {
    const initialBlock = {
      type: "stdout" as const,
      ts: "2026-08-19T00:00:00.000Z",
      text: "first",
      sourceEntryIds: ["stdout-1"],
    };
    const updatedBlock = {
      ...initialBlock,
      ts: "2026-08-19T00:00:01.000Z",
    ...
    expect(flattenChatTranscriptActions([updatedBlock])[0]?.key)
      .toBe(flattenChatTranscriptActions([initialBlock])[0]?.key);
    expect(segmentChatTranscriptBlocks([updatedBlock])[0]?.key)
      .toBe(segmentChatTranscriptBlocks([initialBlock])[0]?.key);
  });

  it("keeps fallback action identities unique across adjacent command groups", () => {
    const blocks = [
      {
        type: "command_group",
        ts: "2026-08-19T00:00:00.000Z",
        items: [{ ts: "2026-08-19T00:00:00.000Z", name: "command_execution" }],
      },
      {
        type: "command_group",
        ts: "2026-08-19T00:00:01.000Z",
        items: [{ ts: "2026-08-19T00:00:01.000Z", name: "command_execution" }],
      },
    ] as unknown as TranscriptBlock[];

    const keys = flattenChatTranscriptActions(blocks).map((action) => action.key);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it("keeps raw transcript identity stable as delta timestamps advance", () => {
    ...
    const liveEntries: TranscriptEntry[] = [initial];
    appendTranscriptEntry(liveEntries, {
      kind: "assistant",
      ts: "2026-08-19T00:00:01.000Z",
      text: " streamed",
      delta: true,
    ...
    expect(transcriptEntryIdentity(liveEntries[0]!)).toBe(transcriptEntryIdentity(initial));
  });

  it("keeps raw keys stable when a trailing limit removes earlier entries", () => {
    const entries: TranscriptEntry[] = [
      { kind: "stdout", ts: "2026-08-19T00:00:00.000Z", text: "first" },
      { kind: "stdout", ts: "2026-08-19T00:00:01.000Z", text: "second" },
      { kind: "stdout", ts: "2026-08-19T00:00:02.000Z", text: "third" },
    ];
    const all = coalesceRawTranscriptEntries(entries);
    const visible = all.slice(-2);

    expect(visible.map(transcriptEntryIdentity)).toEqual(
      all.slice(-2).map(transcriptEntryIdentity),
    );
  });

  it("keeps later turn keys stable when a new turn is inserted before them", () => {
    const firstTurn = {
      kind: "assistant" as const,
      ts: "2026-08-19T00:00:00.000Z",
      text: "First turn",
      sourceEntryId: "turn-1",
    };
    const secondTurn = {
      kind: "assistant" as const,
      ts: "2026-08-19T00:00:02.000Z",
      text: "Second turn",
      sourceEntryId: "turn-2",
    };
    const base = normalizeChatTranscriptTurns([
      firstTurn,
      { kind: "system", ts: "2026-08-19T00:00:01.000Z", text: "turn started" },
      secondTurn,
    ], false);
    const inserted = normalizeChatTranscriptTurns([
      firstTurn,
      { kind: "system", ts: "2026-08-19T00:00:01.000Z", text: "turn started" },
      { kind: "assistant", ts: "2026-08-19T00:00:01.500Z", text: "Inserted turn", sourceEntryId: "turn-inserted" },
      { kind: "system", ts: "2026-08-19T00:00:01.750Z", text: "turn started" },
      secondTurn,
    ], false);

    expect(inserted.turns.map((turn) => turn.key)).toEqual([
      base.turns[0]?.key,
      "turn-source:turn-inserted",
      base.turns[1]?.key,
    ]);
  });

  it("keeps live and persisted projections on the same generation-backed turn key", () => {
    const first = {
      kind: "assistant" as const,
      ts: "2026-08-19T00:00:00.000Z",
      text: "First",
    ...
    appendTranscriptEntry(liveEntries, first);
    appendTranscriptEntry(liveEntries, second);
    const persisted = normalizeChatTranscriptTurns([first, second], true);
    const live = normalizeChatTranscriptTurns(liveEntries, true);

    expect(live.turns[0]?.key).toBe(persisted.turns[0]?.key);
    expect(transcriptBlockIdentity(live.turns[0]!.blocks[0]!)).toBe(
      transcriptBlockIdentity(persisted.turns[0]!.blocks[0]!),
    );
    expect(live.turns[0]?.blocks[0]).toMatchObject({ text: "First streamed" });
  });

  it("keeps identity-less repeated entries unique without using their array index", () => {
    const entries: TranscriptEntry[] = [
      { kind: "system", ts: "2026-08-19T00:00:00.000Z", text: "same event" },
      { kind: "system", ts: "2026-08-19T00:00:01.000Z", text: "same event" },
    ];

    const keys = entries.map(transcriptEntryIdentity);
    expect(new Set(keys).size).toBe(keys.length);
    expect(keys.every((key) => !key.includes("raw-"))).toBe(true);
  });

  it("keeps repeated logical transcript identities distinct by source id", () => {
    const entries: TranscriptEntry[] = [
      {
        kind: "assistant",
        ts: "2026-08-19T00:00:00.000Z",
        text: "same answer",
        sourceEntryId: "answer-1",
      },
      {
        kind: "assistant",
        ts: "2026-08-19T00:00:01.000Z",
        text: "same answer",
        sourceEntryId: "answer-2",
      },
    ];
  ...
  it("does not use an ordinal in normalized turn keys", () => {
    const normalized = normalizeChatTranscriptTurns([
      { kind: "assistant", ts: "2026-08-19T00:00:00.000Z", text: "answer", sourceEntryId: "answer-1" },
    ], false);

    expect(normalized.turns[0]?.key).toBe("turn-source:answer-1");
    expect(normalized.turns[0]?.key).not.toContain("turn-1-");
  });

  it("preserves raw stream anchors across client-side coalescing", () => {
    const entries: TranscriptEntry[] = [
      { kind: "assistant", ts: "2026-08-19T00:00:00.000Z", text: "First", delta: true },
      { kind: "assistant", ts: "2026-08-19T00:00:01.000Z", text: " streamed", delta: true },
    ];
    const coalesced = coalesceRawTranscriptEntries(entries);

    expect(coalesced).toMatchObject([{ text: "First streamed", ts: "2026-08-19T00:00:01.000Z" }]);
    expect(transcriptEntryIdentity(coalesced[0]!)).toBe(
      "assistant:stream:2026-08-19T00:00:00.000Z:phase:",
    );
  });

  it("keeps source-backed raw identity stable across text updates", () => {
    const initial = {
      kind: "assistant" as const,
      ts: "2026-08-19T00:00:00.000Z",
      text: "First",
    ...
    expect(transcriptEntryIdentity(updated)).toBe(transcriptEntryIdentity(initial));
  });
