...
export function createTranscriptSkillTargets(
  ...
  return createTranscriptFileTargets(
    paths.filter((path) => extractSkillSlugFromEntryPath(path)),
    record,
  ).map((target) => ({
    ...target,
    displayLabel: extractSkillSlugFromEntryPath(target.label) ?? target.displayLabel,
  }));
}
...
export function describeToolSemanticInfo(name: string, input: unknown): TranscriptToolSemanticInfo {
  ...
  if (normalizedName === "file_change") {
    ...
    const summary = fileChanges.length > 0
      ? `Edited ${fileChanges.length} ${pluralize("file", fileChanges.length)}`
