...
export function EmptyState({ icon: Icon, message, action, onAction, className }: EmptyStateProps) {
  return (
    <div className={cn("surface-panel mx-auto flex w-full max-w-xl flex-col items-center justify-center rounded-[var(--radius-xl)] px-8 py-14 text-center", className)}>
      ...
      {message ? <p className="mt-3 max-w-md text-sm leading-6 text-muted-foreground">{message}</p> : null}
      {action && onAction && (
