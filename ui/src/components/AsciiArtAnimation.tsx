...
type PaperclipSprite = (typeof RUDDER_SPRITES)[number];
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = RUDDER_SPRITES[(baseCol + baseRow) % RUDDER_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = RUDDER_SPRITES[Math.floor(Math.random() * RUDDER_SPRITES.length)]!;
