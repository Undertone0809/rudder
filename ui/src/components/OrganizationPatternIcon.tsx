...
interface OrganizationPatternIconProps {
...
function makeOrganizationPatternDataUrl(seed: string, brandColor?: string | null, logicalSize = 22, cellSize = 2): string {
...
export function OrganizationPatternIcon({
  organizationName,
  logoUrl,
  brandColor,
  className,
}: OrganizationPatternIconProps) {
  ...
  const patternDataUrl = useMemo(
    () => makeOrganizationPatternDataUrl(organizationName.trim().toLowerCase(), brandColor),
    [organizationName, brandColor],
  );
