    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            If a native picker is unavailable, paste the absolute path (e.g.{" "}
...
/**
 * Shared path picker button. Uses the host-native picker when available and
 * falls back to manual path instructions otherwise.
 */
export function ChoosePathButton({
  ...
  async function handleChoose() {
    setPending(true);
    try {
      const result = await instanceSettingsApi.pickPath({ selectionType });
      if (!result.cancelled && result.path) {
        onPathSelected?.(result.path);
      }
    } catch (error) {
      if (shouldFallbackToManualPathInstructions(error)) {
        setOpen(true);
        return;
      }
      pushToast({
        title: "Couldn't open path picker",
        body: getPathPickerFailureMessage(error),
        tone: "error",
      });
    } finally {
      setPending(false);
  ...
  return (
    <>
      <button
        ...
        disabled={pending}
        className={cn(
          "inline-flex items-center rounded-md border border-border px-2 py-0.5 text-xs text-muted-foreground hover:bg-accent/50 transition-colors shrink-0 disabled:pointer-events-none disabled:opacity-60",
          className,
        )}
        onClick={() => void handleChoose()}
      >
        {pending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Choose"}
      </button>
