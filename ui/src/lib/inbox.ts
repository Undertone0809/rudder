...
export function computeInboxBadgeData({
  ...
  unreadIssues: Issue[];
  attentionChats: ChatConversation[];
  ...
  const chatAttention = attentionChats.length;
  ...
  return {
    inbox: actionableApprovals + joinRequests.length + failedRuns + unreadTouchedIssues + chatAttention + alerts,
