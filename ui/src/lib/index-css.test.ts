...
describe("index.css motion rules", () => {
  ...
  it("keeps the Library sidebar header aligned with the editor tab strip", () => {
    const sidebarHeader = cssBlock(".rudder-doc-editor-sidebar-header");
    const tabsAndBreadcrumbHeader = cssBlock(".rudder-doc-editor-sidebar-header--tabs-and-breadcrumb");

    expect(sidebarHeader).toContain("--rudder-doc-editor-sidebar-header-height: var(--rudder-doc-editor-tab-strip-height)");
    expect(tabsAndBreadcrumbHeader).toContain("--rudder-doc-editor-sidebar-header-height: var(--rudder-doc-editor-tab-strip-height)");
    expect(sidebarHeader).not.toContain("--rudder-doc-editor-sidebar-header-height: calc(var(--rudder-doc-editor-tab-strip-height) - 1px)");
  ...
  it("keeps Library file-tab chrome and sidebar dividers aligned", () => {
    ...
    expect(tabStrip).toContain("--rudder-doc-editor-tab-radius: calc(var(--desktop-workspace-radius) + 1px)");
    expect(tabStrip).toContain("--rudder-doc-editor-tab-corner-size: 8px");
    expect(tabStrip).toContain("--rudder-doc-editor-tab-corner-fill: 4px");
    expect(tabStrip).toContain("--rudder-doc-editor-tab-corner-spread: 3px");
    expect(activeTabCorners).toContain("width: var(--rudder-doc-editor-tab-corner-size)");
    expect(activeTabCorners).toContain("height: var(--rudder-doc-editor-tab-corner-size)");
    ...
    expect(indexCss).not.toContain("var(--rudder-doc-editor-tab-corner-offset)");
    expect(indexCss).toContain("left: calc((var(--rudder-doc-editor-tab-corner-size) - 2px) * -1)");
    expect(indexCss).toContain("right: calc((var(--rudder-doc-editor-tab-corner-size) - 2px) * -1)");
