...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("rudder-worktree-enabled") !== "true") return null;

  const name = readMetaContent("rudder-worktree-name");
  const color = normalizeHexColor(readMetaContent("rudder-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("rudder-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
