...
function createZipArchiveWithDirectoryEntries(rootPath: string) {
  ...
  const entries = [
    ...
    { path: `${rootPath}/ORGANIZATION.md`, body: encoder.encode("# Organization\n"), compressionMethod: 8 },
...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("rudder-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("rudder-demo/ORGANIZATION.md");
    expect(readString(archive, secondOffset + 30 + secondNameLength, secondBodyLength)).toBe("# Organization\n");
  ...
  it("reads a Rudder zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "ORGANIZATION.md": "# Organization\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
  ...
  it("reads standard DEFLATE zip archives created outside Rudder", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("rudder-demo");
