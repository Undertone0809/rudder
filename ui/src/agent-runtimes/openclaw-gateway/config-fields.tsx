...
export function OpenClawGatewayConfigFields({
  ...
  const effectiveHeaders =
    (eff("agentRuntimeConfig", "headers", configuredHeaders) as Record<string, unknown>) ?? {};
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Rudder API URL override">
            <DraftInput
              value={
                eff(
                  "agentRuntimeConfig",
                  "rudderApiUrl",
                  String(config.rudderApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("agentRuntimeConfig", "rudderApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://rudder.example"
            />
          </Field>
          ...
          <Field label="Session strategy">
            <select
              ...
              onChange={(e) => mark("agentRuntimeConfig", "sessionKeyStrategy", e.target.value)}
              className={inputClass}
            >
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("agentRuntimeConfig", "sessionKey", String(config.sessionKey ?? "rudder"))}
                onCommit={(v) => mark("agentRuntimeConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="rudder"
              />
          ...
          <Field label="Role">
            <DraftInput
              value={eff("agentRuntimeConfig", "role", String(config.role ?? "operator"))}
              onCommit={(v) => mark("agentRuntimeConfig", "role", v || undefined)}
              immediate
              className={inputClass}
            ...
            <DraftInput
              ...
              onCommit={(v) => {
                const parsed = v
                  ...
                  .filter(Boolean);
                mark("agentRuntimeConfig", "scopes", parsed.length > 0 ? parsed : undefined);
              }}
              immediate
              className={inputClass}
          ...
          <Field label="Wait timeout (ms)">
            <DraftInput
              value={eff("agentRuntimeConfig", "waitTimeoutMs", String(config.waitTimeoutMs ?? "120000"))}
              onCommit={(v) => {
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Rudder persists a device key during onboarding so pairing approvals
