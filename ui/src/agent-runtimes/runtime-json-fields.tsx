...
function updateJsonConfig(
  ...
  next: string,
  set: AgentRuntimeConfigFieldsProps["set"],
  mark: AgentRuntimeConfigFieldsProps["mark"],
  configKey: string,
) {
  if (isCreate) {
    set?.({ [key]: next });
  ...
  if (!trimmed) {
    mark("agentRuntimeConfig", configKey, undefined);
    return;
  }
  ...
  try {
    ...
    if (typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) {
      mark("agentRuntimeConfig", configKey, parsed);
    }
  } catch {
