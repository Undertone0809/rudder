...
These tools are automatic runtime infrastructure, not custom integrations. Rudder prepares their configuration and run identity; operators do not manually connect or configure them. The bundled `rudder` CLI skill remains the fallback when a runtime cannot use the managed tool surface.
