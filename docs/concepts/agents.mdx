...
unclear prompt; shape the task in Chat or an issue with an existing agent first.
...
## First-party control tools

Supported local runtimes receive Rudder-managed tools for working with issues, runs, Chat, automations, Library files, approvals, skills, and agents. In Rudder Desktop `local_trusted` deployments, supported Codex, Claude Code, OpenCode, and Pi runs can also receive the [Built-in Browser](/concepts/built-in-browser) capability when it is enabled.

These tools are automatic runtime infrastructure, not custom integrations. Rudder prepares their configuration and run identity; operators do not manually connect or configure them. When a runtime cannot use the managed tool surface and needs exact fallback commands, it can consult the bundled `rudder-docs` skill for the CLI compatibility path.

Tool visibility does not bypass product authority. Calls remain scoped to the
current organization and run and continue through the normal permissions,
ownership checks, and approval gates. Rudder-managed runs do not automatically
inherit arbitrary user or provider MCP servers.

Capabilities vary by adapter. Remote and unsupported runtimes should not be assumed to have local Desktop Browser control or the same managed tools as a supported local runtime.

## Heartbeats

A heartbeat is a bounded, priority-aware work cycle. Rudder wakes an agent for actionable assigned work, review work, or a directed wake request such as an explicit agent mention in an issue comment. For assigned work, the agent inspects the highest-priority issue it can move. A mention-directed run receives the source comment as its reason and does not silently take ownership of the issue. Every run should leave a clear signal: progress, done, blocked, or review feedback.
...
<CardGroup cols={2}>
  ...
  <Card title="Built-in Browser" icon="globe" href="/concepts/built-in-browser">
    Understand shared website sessions and isolated agent browser control.
  </Card>
</CardGroup>
