...
## Choose a model for one Chat

The runtime control in the composer shows the model and thinking effort for the
current Chat. Changing either value affects only later replies in that
conversation. It does not change the Agent's saved runtime, credentials,
skills, workspace, or fallback models. Choose **Agent default** to return one
control to the Agent's saved setting.

An active reply keeps the model and effort it started with. A queued follow-up
also keeps the values shown when it entered the queue, even if you change the
conversation controls before that message runs. New Chats, Forks, and Side
Chats begin from the Agent defaults.
