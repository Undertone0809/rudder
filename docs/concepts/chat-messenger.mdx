...
message. You can add an optional comment and files to each annotation. Use
**Add to chat** to attach the selection directly, **More details** to include an
explanation prompt, or **Ask in side chat** when the source is a completed
assistant response.

Draft annotations remain editable. After you send the message, they become
immutable message evidence. Rudder preserves them through queued follow-ups,
Steer, retry, edit branching, Fork, and Side Chat, and lets you return to the
original source when it is still available.
