...
Review the **Issue Key** before creating the organization. Rudder derives a
short key from the organization name and previews the first readable issue id,
such as `R6-1`. The key must begin with a letter, use only uppercase letters and
digits, and be unique in the Rudder instance.

The organization name, its stable URL, and its Issue Key have separate jobs.
Renaming the organization does not silently change its URL or issue ids. You can
change the Issue Key explicitly in organization settings later; existing issue
numbers stay the same, and links that use an older Issue Key continue to resolve.
