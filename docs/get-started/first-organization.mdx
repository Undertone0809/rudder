...
description: "Run the first Rudder work loop: create an organization, choose Chat or an issue, run an agent, review evidence, and preserve feedback."
icon: "building-2"
keywords:
...
An organization is Rudder's first work surface. It is where humans set direction, move tasks forward through Chat or issues, and leave inspectable work records through conversations, comments, runs, reviews, and timelines.
...
organization, choose a conversational or structured workflow, run the agent,
inspect the evidence, and use Messenger to understand what needs attention.
...
The goal is not to click every button. You are completing a task through an inspectable work object. In Rudder, that can be a Chat conversation or an `issue`.
...
Rudder derives an internal **Issue Key** from the organization name. It uses
that key to create readable issue ids such as `R6-1`, while keeping the first
organization setup focused on the organization name.

The organization name, its stable URL, and its Issue Key have separate jobs.
Renaming the organization does not silently change its URL or issue ids, and
the Issue Key is not exposed in regular organization settings. Compatibility
migrations preserve old Issue Key links when a key must change. A
new-organization import lets you choose a fresh key, but does not copy Issue Key
aliases from the source organization.

![Rudder dashboard overview](../images/board-overview.png)

## 2. Choose Chat or an issue

Chat and issues can both carry real agent work from request to result. Chat is conversation-driven; an `issue` adds structured status, ownership, priority, dependencies, and review state. Choose the style that fits the task and your preference.

This walkthrough uses an issue to teach the structured workflow. You can run the same small task in Chat instead, and you do not need to convert it merely because the agent spends time or produces a durable result.
...
## 7. Use Chat as a task workflow

Use Chat when you prefer to move the task forward through an ongoing conversation. It supports explaining the request, attaching context, choosing an agent, running the work, inspecting outputs, and refining the result without requiring an issue.

![Chat issue proposal](../images/chat-create-issue-proposal.png)

A good pattern is:

1. Start in Chat with the request.
2. Let the selected agent execute the task, clarifying or refining in the same conversation when needed.
3. Inspect the run and outputs in the conversation.
4. Refine or complete the task in Chat.
5. Create or link an issue only when you want structured coordination.

Creating an issue adds fields and team coordination; it does not make the task more real. A team policy may still require issue structure for governed workflows.
...
Messenger should bring you back to the native object that owns the next action, whether that is a Chat, issue, run, review, or approval.
...
2. A task moves forward through Chat or an issue.
3. The chosen surface preserves the request and context.
4. The agent run leaves evidence.
5. A reviewer or owner accepts, blocks, or requests changes when needed.
6. Messenger shows attention needs.
7. The Chat or issue preserves the final record and linked outputs.
...
<CardGroup cols={2}>
  ...
  <Card title="Chat and Messenger" icon="messages-square" href="/concepts/chat-messenger">
    Understand how conversational task work and attention recovery fit together.
  </Card>
</CardGroup>
