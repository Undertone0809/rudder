...
Daily session rollover 默认开启，窗口为 24 小时。外部飞书 chat 不会改变；超过窗口后，下一条 inbound message 到达时，Rudder 才会创建并绑定一条新的对话。这个切换是惰性的，安静的飞书 chat 不会在后台产生空 session。

如果 Agent reply 仍在运行或收尾，这条 inbound message 会继续留在当前 session，rollover 等后续消息再处理。你可以选择是否向飞书发送一条简短的 `New daily session started.` 通知。需要立即开始新的 Rudder 对话时，直接使用 `/new`，不用等待 daily boundary。
