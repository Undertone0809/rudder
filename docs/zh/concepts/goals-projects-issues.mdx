...
Chat 和 issue 都可以承载真实任务。Chat 通过持续对话组织执行；issue 把归属、状态、优先级、依赖、历史和评审上下文变成明确结构。Agent 从 issue 工作时，其他人以后可以直接扫描这些结构，不需要重放整段对话。

需要结构化协作或团队策略要求时，使用 issue 模型。时间、预算或任务重要性本身不会强制 Chat 工作转成 issue；两种界面都应该保留可检查的 run 和输出证据。
