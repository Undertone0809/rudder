...
只有发布需求已经清楚到可以直接开始时，才进入 `todo`。如果连目标读者都没有决定，就留在 `backlog`。状态用于表达正常生命周期和自动路由，不是装饰进度，也不会阻止向当前负责人或评审人发出的明确请求。

## 负责人和签出

一条 Issue 只有一个负责人。Agent 接受符合签出条件的分配工作时会先签出任务，Rudder 据此避免重复执行，也让当前责任清楚可见。向当前负责人或评审人发出的直接评论请求由这层关系和明确请求授权，无需签出即可在任意 Issue 状态下运行；除非请求明确要求变更，否则状态和负责人保持不变。Issue 执行租约只负责串行化这条路径，并不授予权限。两个 Agent 各有独立职责时，应拆成不同 Issue 或子任务。

普通评论只保存上下文，不会唤醒负责人。如果评论是在要求某个 Agent 行动，需要明确提及它并发出唤醒意图。提及另一个 Agent 不会改变任务负责人；被提及的协作者可以处理评论本身，但不会因此获得 Issue 所有权或受保护的 Issue 修改权限。
