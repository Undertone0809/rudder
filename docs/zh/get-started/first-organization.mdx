...
Rudder 会根据组织名称自动生成内部 **Issue Key**，并用它形成 `R6-1` 这样的可读 issue 编号。首次创建组织时，用户只需要填写组织名称，不需要理解或选择 Issue Key。

组织名称、稳定 URL 和 Issue Key 各自负责不同的身份。修改组织名称不会顺带改变 URL 或 issue 编号，常规组织设置也不会暴露 Issue Key。兼容迁移更换 key 时，旧 Issue Key 的链接仍然可以打开同一条 issue。导入为新组织时可以选择新的 key，但不会复制源组织的历史 Issue Key alias。
