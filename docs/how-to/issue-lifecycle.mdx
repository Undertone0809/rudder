...
Issues are Rudder's structured path for inspectable agent work, available alongside conversation-driven Chat. This guide focuses on the issue journey: when to create one, when to assign it, when to add a reviewer, how to set status, and how to close the loop.
