...
Daily session rollover is enabled by default with a 24-hour window. The Feishu
chat itself stays the same, but Rudder starts a new bound conversation when the
next inbound message arrives after that window. The rollover is lazy: quiet
chats do not create empty future conversations in the background.

If an agent reply is still active or closing, the inbound message stays in the
current session and rollover waits for a later message. You can choose whether
Feishu receives a short `New daily session started.` notice. Use `/new` when you
want to start a new Rudder conversation immediately instead of waiting for the
daily boundary.
