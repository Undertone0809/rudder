...
These tools cover Rudder control-plane operations such as issues, runs, Chat, automations, Library files, approvals, skills, and agents. In a Desktop `local_trusted` deployment, supported local runs can also receive [Built-in Browser](/concepts/built-in-browser) tools when that instance capability is enabled.
