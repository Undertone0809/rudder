...
An agent runtime is the execution backend Rudder calls when an agent wakes up. Rudder owns the organization, Chat or issue context, Agent Run, transcript, and close-out evidence. The runtime owns the actual work: running a local CLI, launching a custom process, calling an HTTP endpoint, or forwarding the run to a gateway.
...
7. Run one low-risk Chat turn, or assign a low-risk issue and run one heartbeat.
...
For Pi + DeepSeek, choose or enter `deepseek/deepseek-chat` in Rudder. The
onboarding form will show a `DEEPSEEK_API_KEY` field, store the value as a
Rudder organization secret, and reference that secret from the agent runtime
config. Native DeepSeek also works if you already ran `pi /login` locally. Some
Pi model IDs route through OpenRouter unless the native provider is selected; if
Pi reports `No API key found for openrouter`, either set `OPENROUTER_API_KEY` or
add a native DeepSeek provider/model in `~/.pi/agent/models.json`. Use **Test
now** as the source of truth.
...
Codex-specific options include web search, sandbox bypass, and subscription cost
estimation. Enable search only when the agent should use web results during
runs. Enable sandbox bypass only for trusted local work that needs broader
filesystem or network access. **Estimate subscription usage cost** is on by
default, so ChatGPT/Codex subscription-authenticated runs appear in Rudder as
API-equivalent spend instead of `$0.00`; Rudder estimates that spend from the
model token usage and the built-in OpenAI/Codex price table. Known-model
estimates count toward Rudder spend and budget hard stops. Turn it off only for
agents whose subscription usage should remain `$0.00`.

Claude Code options include Chrome access, permission behavior, and max turns per run. Keep these tight for routine work; loosen them only when the agent needs that capability. Claude Code's Chrome option is separate from Rudder Desktop's Built-in Browser.

### Rudder-managed control tools

For supported local runtimes, Rudder automatically prepares its first-party control tools for each run. Do not create a custom MCP integration or manually add a Rudder MCP server to the agent:

- Codex, Claude Code, and OpenCode local receive Rudder-managed MCP configuration.
- Pi local receives the equivalent Rudder-managed native tool bridge.
- When managed tools are unavailable and exact fallback commands are needed, consult the bundled `rudder-docs` skill for the CLI compatibility path.

These tools cover Rudder operations such as issues, runs, Chat, automations, Library files, approvals, skills, and agents. In a Desktop `local_trusted` deployment, supported local runs can also receive [Built-in Browser](/concepts/built-in-browser) tools when that instance capability is enabled.

Seeing a managed tool does not grant new authority. Every call stays scoped to
the current organization and run and still passes the normal permission,
ownership, and approval checks for that action. Rudder also does not
automatically inherit arbitrary user or provider MCP servers into this managed
run surface.

Tool availability is runtime-specific. Remote, gateway, generic process, and unsupported runtimes do not inherit a promise of managed tools or local Desktop Browser control. Use the environment test and a real run as evidence for the runtime you selected.
...
A passing environment test is not the final proof. It only means Rudder can plausibly launch or contact the runtime. The real proof is one Chat turn or issue heartbeat that leaves inspectable evidence:

- the run starts under the expected runtime
- transcript entries or raw output appear on the run
- the source Chat or issue receives a visible result, progress, blocked, review, or done signal
...
- the first real Chat turn or issue heartbeat leaves evidence on the source Chat or issue
...
<CardGroup cols={2}>
  ...
  <Card title="Built-in Browser" icon="globe" href="/concepts/built-in-browser">
    Review Browser availability and trust boundaries.
  </Card>
</CardGroup>
