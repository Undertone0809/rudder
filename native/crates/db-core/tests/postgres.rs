...
#[tokio::test]
#[ignore = "set RUDDER_DB_CORE_DATABASE_URL, RUDDER_DB_CORE_SCHEMA, and RUDDER_DB_CORE_ORG_ID for PostgreSQL integration"]
async fn reads_bounded_activity_and_run_evidence_without_writes() {
    use rudder_db_core::{
        OrganizationScope,
        activity_read::{
            ActivityListFilter, ActivityPageRequest, ActivityRunReadRepository, RunListFilter,
    ...
    let pool = PgPoolOptions::new()
        ...
        .await
        .expect("connect to integration database");
    let repository = ActivityRunReadRepository::new(pool.clone());
    let scope = TrustedOrganizationScope::from_host(
        OrganizationScope::single(org_id).expect("valid organization scope"),
    );
    let page = ActivityPageRequest::new(10).expect("valid page");
    repository
        .list_activity(&scope, ActivityListFilter::default(), page.clone())
        .await
        .expect("read bounded activity");
    repository
        .list_runs(&scope, RunListFilter::default(), page)
        .await
        .expect("read bounded run summaries");
    pool.close().await;
}
