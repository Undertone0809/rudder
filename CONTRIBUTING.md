...
New Rudder contributions are made under the repository's Apache-2.0 project license posture.

Rudder began from an early Paperclip fork, and preserved upstream-derived portions continue to carry the original MIT notice where applicable.

See `LICENSE`, `NOTICE`, and `LICENSES/MIT-PAPERCLIP` for the canonical repository-level texts.
...
- **First** open a GitHub Discussion (or issue, if it's clearly scoped)
...
> - This pull request adds per-adapter configuration of compaction, either auto or rudder managed
> - That way we can get optimal performance from any adapter/provider in Rudder
...
Questions? Open a GitHub Discussion — we're happy to help.
