...
const migrationPreflightBinaryName = process.platform === "win32" ? "migration-preflight.exe" : "migration-preflight";
const serverFoundationBinaryName = process.platform === "win32" ? "rudder-server-foundation.exe" : "rudder-server-foundation";
...
async function main() {
  ...
  await fs.access(serverFoundationSourcePath);
  ...
  await fs.copyFile(serverFoundationSourcePath, serverFoundationDestinationPath);
  ...
  if (process.platform !== "win32") await fs.chmod(migrationPreflightDestinationPath, 0o755);
  if (process.platform !== "win32") await fs.chmod(serverFoundationDestinationPath, 0o755);
